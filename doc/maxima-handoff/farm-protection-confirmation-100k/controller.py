...
BASE=Path(__file__).resolve().parent;ROOT=Path('/home/bradley/glob2-maxima-mainline-100k-final');REMOTE=ROOT/'output/farm-protection-confirmation-100k'
...
try:
 ...
 if json.loads((BASE/'STATUS.json').read_text()).get('status')=='stage_complete':
  import gzip,maxima_win_report as report
  p=json.loads((BASE/'protocol.json').read_text());m=json.loads((BASE/'manifest.json').read_text());results={}
  for f in (BASE/'results').glob('*/result.json.gz'):
   with gzip.open(f,'rt') as stream:r=json.load(stream)
   results[r['execution_id']]=r
  report.publish_final(BASE/'CONFIRMATION_RESULT.json',p,m,results)
  for h in e.HOSTS:fleet.remote(h,ROOT,REMOTE,'stop',value={'reason':'Fixed farm-protection confirmation complete; no automatic next stage','active_engines':'none; all results collected'})

...
BASE=Path(__file__).resolve().parent;ROOT=Path('/home/bradley/glob2-maxima-mainline-100k-final');REMOTE=ROOT/'output/farm-protection-pilot-100k'
...
try:
 ...
 if json.loads((BASE/'STATUS.json').read_text()).get('status')=='stage_complete':
  for h in e.HOSTS:fleet.remote(h,ROOT,REMOTE,'stop',value={'reason':'Farm-protection pilot complete; awaiting confirmation plan','active_engines':'none; all results collected'})

...
BASE=Path(__file__).resolve().parent;ROOT=Path('/home/bradley/glob2-maxima-defense-fix');REMOTE=ROOT/'output/farming-sub-switches-confirmation'
...
try:
 ...
 if json.loads((BASE/'STATUS.json').read_text()).get('status')=='stage_complete':
  for h in e.HOSTS:fleet.remote(h,ROOT,REMOTE,'stop',value={'reason':'All eight fixed confirmations complete; final report ready','active_engines':'none'})

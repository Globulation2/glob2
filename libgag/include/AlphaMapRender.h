...
namespace GAGCore
{
inline void drawAlphaMapBatched(const std::valarray<unsigned char> &map, int mapW, int mapH, int x, int y,
                                int cellW, int cellH, unsigned char red, unsigned char green,
                                unsigned char blue)
{
    if (mapW < 2 || mapH < 2)
        return;
    ...
    for (int dy = 0; dy < mapH - 1; ++dy)
    {
        ...
        for (int dx = 0; dx < mapW - 1; ++dx)
        {
            ...
            Vertex corners[4] = {{float(left), float(top), red, green, blue, tl},

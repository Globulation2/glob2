...
class CooperativeTask
{
  ...
  public:
	...
	struct promise_type
	{
		// Awaited tasks belong to their suspended parents; the root owns this state.
		State ownedState;
	...
	{
		if (!handle.done())
			handle.promise().state->leaf.resume();
		return handle.done();
	}
	bool result() const
	{
		if (!handle.done())
			throw std::logic_error("Job is not complete");
	...
	struct Checkpoint
	{
		...
		void await_suspend(Handle handle) noexcept
		{
			...
			if (stage)
				handle.promise().state->stage = stage;
		}
		void await_resume() noexcept {}
	};

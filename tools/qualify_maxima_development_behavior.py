#!/usr/bin/env python3
"""Qualify development switch fixtures against unchanged, frozen engine objects."""
import argparse
import json
import os
...
def main():
    ...
    source = exp.ROOT / 'test/MaximaDevelopmentSwitchBehaviorTest.cpp'
    ...
    exp.atomic(args.output/'PASS.json', {
        ...
        'validated_switches': ['repairs.enabled','upgrades.enabled'], 'cases': 16,
        'checks': ['paired on/off', 'building damage across all three levels and school eligibility', 'inactive-world silence',
                   'repair/upgrade selection and concrete OrderConstruction generation'],
        'parent_disabled': 'not applicable: independent development switches',
    ...
    print('repairs.enabled and upgrades.enabled: sixteen behavioral cases PASS')

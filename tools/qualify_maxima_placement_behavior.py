#!/usr/bin/env python3
"""Qualify placement switch fixtures against unchanged, frozen engine objects."""
import argparse
import json
import os
...
def main():
    ...
    source = exp.ROOT / 'test/MaximaPlacementSwitchBehaviorTest.cpp'
    ...
    exp.atomic(args.output/'PASS.json', {
        ...
        'validated_switches': ['placement.food_preservation_enabled','placement.defensive_siting_enabled','placement.spacing_compactness_enabled','placement.artery_routing_enabled'], 'cases': 16,
        'checks': ['paired ON/OFF', 'actual planner selection changes under farm capacity, threat and spacing controls', 'artery routing path creation', 'no construction authority negative control'],
        'parent_disabled': 'all four tested with construction authority zero',
    ...
    print('Four placement switches: 16 behavioral cases PASS')

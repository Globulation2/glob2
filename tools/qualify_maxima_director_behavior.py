#!/usr/bin/env python3
"""Qualify economy/staffing switch fixtures against unchanged, frozen engine objects."""
import argparse
import json
import os
...
def main():
    ...
    source = exp.ROOT / 'test/MaximaDirectorSwitchBehaviorTest.cpp'
    ...
    exp.atomic(args.output/'PASS.json', {
        ...
        'validated_switches': ['postures.expand_enabled','postures.develop_enabled','postures.mobilize_enabled','postures.campaign_enabled','postures.finish_enabled','postures.defend_enabled','postures.recover_enabled','emergencies.food_enabled','emergencies.colony_enabled','colonization.enabled','fruit.enabled'], 'cases': 88,
        'checks': ['paired ON/OFF','posture selection and forced exit from disabled current posture','emergency activation and posture override','builder-gated colony intents, including during recovery','fruit flag orders, absent resources and recovery suppression'],
        'parent_disabled': 'emergency posture override tested with its posture disabled; fruit suppressed by recovery; other controls independent',
    ...
    print('Eleven director switches: 88 behavioral cases PASS')

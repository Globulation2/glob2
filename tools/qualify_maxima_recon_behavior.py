#!/usr/bin/env python3
"""Qualify reconnaissance switch fixtures against unchanged, frozen engine objects."""
import argparse
import json
import os
...
def main():
    ...
    source = exp.ROOT / 'test/MaximaReconSwitchBehaviorTest.cpp'
    ...
    exp.atomic(args.output/'PASS.json', {
        ...
        'validated_switches': ['recon.enabled','recon.scouting_missions_enabled','recon.economic_watch_enabled','recon.force_memory_enabled'], 'cases': 40,
        'checks': ['paired ON/OFF', 'actual exploration flag creation', '80-percent exploration threshold and colony emergency suppression', 'economic watch orders gated by prestige and observed activity', 'constructor-resolved force memory with visible, hidden, and expired observations'],
        'parent_disabled': 'recon/scouting gates tested together; economic watch tested with scouting disabled; force memory is observational and unused when recon disabled',
    ...
    print('Four recon switches: 40 behavioral cases PASS')

...
for name in ['pool0b0','school1b0']:
    for f in m['frames']:
        if f['id']==name and not f['recipe'].startswith('recovered original building:'):
            for l in f['layers']:assert digest(PACK/l['file'])==digest(ROOT/'datasrc/gfx/production/ai-upscaled'/l['file'])
...
for level,l in enumerate(m['resource_atlas']['levels']):
    ...
    for i in range(65):
        ...
        if i not in set(range(10)) | {10,11,12,13,15,16,17,18,20,21,22,23,24}:
            ...
            assert record['recipe'].startswith(('recovered original tree:', 'recovered original wheat:', 'recovered original papyrus:'))
...
print('PASS: all 65 resource frames, original native tree/wheat/papyrus alpha / constrained other alpha, isolated resource atlas mip levels')

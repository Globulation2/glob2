...
BASE={'water':50,'sand':0,'grass':50,'desert':0,'smooth':4,'river':50,'craters':50,'extra':0,'island':50,'beach':1,'fruit':4,'w':7,'h':7,'teams':4}
...
def sample(task):
    ...
    params=config.get('params',{}) if config.get('preset') else BASE|config.get('params',{})
    command=[str(ROOT/'build/src/MapGeneratorStudy'),str(config['method']),str(seed),p,'tuning']
    if config.get('preset'): command += ['preset']
...
def run(configs,count,start,label,workers=8):
    ...
    for config in configs:
        for seed in sorted({start,start+count//2,start+count-1}):
            a,b=sample((config,seed)),sample((config,seed))
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--configs',required=True);p.add_argument('--output',default=str(OUT));p.add_argument('--count',type=int,default=32);p.add_argument('--start',type=int,default=10001);p.add_argument('--label',default='sweep');p.add_argument('--workers',type=int,default=8)
    a=p.parse_args();OUT=Path(a.output);OUT.mkdir(parents=True,exist_ok=True);c=json.loads(Path(a.configs).read_text())

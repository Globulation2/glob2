#!/usr/bin/env python3
"""Qualify raiding switch fixtures against unchanged, frozen engine objects."""
import argparse
import json
import os
...
def main():
    ...
    source = exp.ROOT / 'test/MaximaRaidingSwitchBehaviorTest.cpp'
    ...
    exp.atomic(args.output/'PASS.json', {
        ...
        'validated_switches': ['raiding.enabled'], 'cases': 8,
        'checks': ['paired on/off', 'visible worker-cluster eligibility', 'inactive-world silence',
                   'raid authorization, mission selection and issued flag-building orders'],
        'parent_disabled': 'tactics.enabled=false blocks raid authorization',
    ...
    print('raiding.enabled: eight behavioral cases PASS')

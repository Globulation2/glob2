...
class WssTransport final : public NetTransport
{
	struct Session
	{
		...
		Session(const std::string &address, uint16_t port)
		{
			...
			if (address.rfind("wss://", 0) != 0)
				throw std::invalid_argument("Expected wss origin");
			...
			if (authority.front() == '[')
			{
				...
				if (end == std::string::npos)
					throw std::invalid_argument("Invalid IPv6 origin");
			...
			if (host.empty() || service.empty() ||
				service.find_first_not_of("0123456789") != std::string::npos ||
  ...
  public:
	void open(const std::string &address, uint16_t port) override
	{
		...
		try
		{
			session = std::make_unique<Session>(address, port);
		}
	...
	{
		if (state() != State::Connected || bytes.size() > queueLimit - session->outgoingBytes)
			return false;

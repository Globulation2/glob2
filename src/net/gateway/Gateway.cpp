...
class Session : public std::enable_shared_from_this<Session>
{
	...
	void reply(http::status status, std::string body)
	{
		...
		http::async_write(ws.next_layer(), response,
						  [self = shared_from_this()](Error, size_t) { self->stop(); });
	}
	...
	void connectBackend(const std::string &host, const std::string &port)
	{
		...
		resolver.async_resolve(
			host, port,
			[self = shared_from_this()](Error ec, tcp::resolver::results_type endpoints)
			{
				if (ec)
				{
					self->reply(http::status::bad_gateway, "Backend unavailable\n");
					return;
				}
				self->backend.async_connect(
					endpoints,
					[self](Error error, const tcp::resolver::results_type::endpoint_type &)
					{
						...
						self->ws.binary(true);
						self->ws.async_accept(self->parser.get(),
											  [self](Error accepted)
											  {
												  if (accepted)
												  {
	...
	{
		ws.async_read(input,
					  [self = shared_from_this()](Error ec, size_t length)
					  {
						  if (ec || !self->ws.got_binary())
						  {
							  self->stop();
							  return;
						  }
						  ...
						  asio::async_write(self->backend.socket(), self->input.data(),
	...
	void readBackend()
	{
		backend.async_read_some(asio::buffer(backendBuffer),
								[self = shared_from_this()](Error ec, size_t length)
								{
									if (ec)
									{
									...
									self->ws.async_write(
										asio::buffer(self->backendBuffer.data(), length),
										[self](Error error, size_t)
										{
											if (error)
											{
  ...
  public:
	Session(tcp::socket socket, const Configuration &configuration,
			std::shared_ptr<Metrics> counters)
		: ws(std::move(socket)), backend(ws.get_executor()), resolver(ws.get_executor()),
		  config(configuration), metrics(counters)
	{
		++metrics->active;
	...
	void run()
	{
		...
		http::async_read(
			ws.next_layer(), input, parser,
			[self = shared_from_this()](Error ec, size_t)
			{
				if (ec)
				{
					self->stop();
				...
				{
					self->reply(http::status::ok, "ok\n");
					return;
				}
				if (request.target() == "/metrics")
				{
					self->reply(http::status::ok,
								"glob2_gateway_connections " +
									std::to_string(self->metrics->active) + "\n" +
									"glob2_gateway_accepted_total " +
									std::to_string(self->metrics->accepted) + "\n" +
									"glob2_gateway_rejected_total " +
									std::to_string(self->metrics->rejected) + "\n" +
									"glob2_gateway_bytes_to_backend_total " +
									std::to_string(self->metrics->bytesToBackend) + "\n" +
									"glob2_gateway_bytes_to_client_total " +
									std::to_string(self->metrics->bytesToClient) + "\n");
					return;
				}
				const auto origin = request[http::field::origin];
				if (!websocket::is_upgrade(request) ||
					(!origin.empty() && origin != self->config.origin))
				{
					++self->metrics->rejected;
					self->reply(http::status::forbidden, "Upgrade or origin rejected\n");
					return;
				}
				if (request.target() == "/yog")
					self->connectBackend(self->config.lobbyHost, self->config.lobbyPort);
				else if (request.target() == "/router")
					self->connectBackend(self->config.routerHost, self->config.routerPort);
				else
				{
					++self->metrics->rejected;
					self->reply(http::status::not_found, "Unknown backend\n");
				}
			});
	}
};
  ...
  public:
	...
	void run()
	{
		acceptor.async_accept(
			[this](Error ec, tcp::socket socket)
			{
				if (!ec)
				{
					if (metrics->active < config.limit)
						std::make_shared<Session>(std::move(socket), config, metrics)->run();
					else
					{
						++metrics->rejected;
						Error ignored;
		...
		for (int i = 1; i < argc; ++i)
		{
			...
			if (option == "--help")
			{
				std::cout << "glob2-ws-gateway [--listen ADDRESS] [--port PORT] [--origin ORIGIN] "
							 "[--lobby-host HOST] [--lobby-port PORT] [--router-host HOST] "
							 "[--router-port PORT]\n";
				return 0;
			}
			if (++i == argc)
			...
			else if (option == "--port")
			{
				...
				config.origin = value;
			else if (option == "--lobby-host")
				config.lobbyHost = value;
			else if (option == "--lobby-port")
				config.lobbyPort = value;
			else if (option == "--router-host")
				config.routerHost = value;
			else if (option == "--router-port")

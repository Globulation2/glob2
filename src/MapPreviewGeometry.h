...
struct MapPreviewGeometry
{
    ...
    {
        if (mw<=0 || mh<=0 || box.w<=0 || box.h<=0) return {box.x,box.y,0,0};
        ...
        if (std::abs(double(box.w)*mh-double(box.h)*mw)<std::max(mw,mh)) return box;
    ...
    void drag(double dx,double dy,Rect area)
    {
        ...
        { offsetX=wrap(offsetX+dx/area.w); offsetY=wrap(offsetY+dy/area.h); }
    }

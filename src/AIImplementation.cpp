...
AIImplementation::AIImplementation(Player *player)
{
	init(player);
}

AIImplementation::AIImplementation(SDL_RWops *stream, Player *player)
{
	init(player);
	load(stream);
}

void AIImplementation::init(Player *player)
{
	timer=0;
	phase=0;
	phaseTime=1024;
...
bool AIImplementation::load(SDL_RWops *stream)
{
	assert(player);
...
void AIImplementation::save(SDL_RWops *stream)
{
...
Order *AIImplementation::getOrder(void)
{
	timer++;
...
int AIImplementation::estimateFood(int x, int y)
{
	int rx, ry;
...
int AIImplementation::countUnits(void)
{
...
int AIImplementation::countUnits(const int medicalState)
{
	Unit **myUnits=player->team->myUnits;
	int c=0;
	for (int i=0; i<1024; i++)
...
Order *AIImplementation::swarmsForWorkers(const int minSwarmNumbers, const int nbWorkersFator, const int workers, const int explorers, const int warriors)
{
...
void AIImplementation::nextMainBuilding(const int buildingType)
{
...
bool AIImplementation::checkUIDRoomForBuilding(int px, int py, int width, int height)
{
	Game *game=player->team->game;
...
int AIImplementation::nbFreeAround(const int buildingType, int posX, int posY, int width, int height)
{
	Game *game=player->team->game;
...
bool AIImplementation::parseBuildingType(const int buildingType)
{
	return (buildingType==BuildingType::DEFENSE_BUILDING);
}

void AIImplementation::squareCircleScann(int &dx, int &dy, int &sx, int &sy, int &x, int &y, int &mx, int &my)
{
	if (x>=mx)
	{
		dx=0;
...
bool AIImplementation::findNewEmplacement(const int buildingType, int *posX, int *posY)
{
	Building **myBuildings=player->team->myBuildings;
	Building *b=myBuildings[mainBuilding[buildingType]];
	if (b==NULL)
...
Order *AIImplementation::mayAttack(int critticalMass, int critticalTimeout, Sint32 numberRequested)
{
	Unit **myUnits=player->team->myUnits;
	int ft=0;
	{
...
Order *AIImplementation::adjustBuildings(const int numbers, const int numbersInc, const int workers, const int buildingType)
{
	Building **myBuildings=player->team->myBuildings;
	//Unit **myUnits=player->team->myUnits;
	int fb=0;
...
Order *AIImplementation::checkoutExpands(const int numbers, const int workers)
{
	//Building **myBuildings=player->team->myBuildings;
	std::list<Building *> swarms=player->team->swarms;
	int ss=swarms.size();
...
Order *AIImplementation::mayUpgrade(const int ptrigger, const int ntrigger)
{
	Building **myBuildings=player->team->myBuildings;

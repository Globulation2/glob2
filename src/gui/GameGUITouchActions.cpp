...
void GameGUITouch::drawBuildingActions()
{
    ...
    for (size_t i=0;i<rows.size();++i) {
        ...
        gfx->drawFilledRect(int(rect.x),int(rect.y),int(rect.w),int(rect.h),row.selected ? PhoneTheme::selected : PhoneTheme::field);
        if (row.kind==0) {
...
void GameGUITouch::tapBuildingAction(ViewPoint point)
{
    ...
    if (row.kind==0) {
        ...
        if (row.kind==3) gui.requestBuildingConstruction(*b);
        else gui.requestBuildingDestruction(*b);

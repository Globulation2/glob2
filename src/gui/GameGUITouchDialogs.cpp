...
void GameGUITouch::prepareDialog()
{
    ...
    const double textScale=PhoneTheme::textScale*globalContainer->settings.mobileDialogTextPercent/100.0;
...
bool GameGUITouch::drawDialog()
{
    ...
    const double textScale=PhoneTheme::textScale*globalContainer->settings.mobileDialogTextPercent/100.0;
    gfx->setClipRect(); gfx->drawFilledRect(0,0,gfx->getW(),gfx->getH(),PhoneTheme::paper);

...
void GameGUITouch::clampScroll()
{
    ...
    if(showsBuildPalette()) { panelScroll=std::clamp(panelScroll,0.0,std::max(0.0,gui.buildingsChoiceName.size()*64.0-content.h/globalContainer->gfx->logicalUnitsPerPoint()));return; }
    ...
    for (const auto& action : changes) {
        ...
        if (interfaceGesture) {
            ...
            if (usesHUD() && ownerRegion==3 && action.kind==TouchActionKind::Pan) {
                ...
                else panelScroll-=point.y/(showsBuildPalette()?globalContainer->gfx->logicalUnitsPerPoint():panelScale());
                clampScroll();
            }
        ...
        if (action.kind==TouchActionKind::Pan) {
            ...
            if (action.factor>0) gui.zoomMap(std::log(action.factor)/std::log(1.1),int(point.x),int(point.y));
        } else if (action.kind==TouchActionKind::Preview && world().contains(point) && !controls().contains(point)) {
            preview=ViewPoint{double(gui.mapMouseX(point.x)+gui.viewportX*32),double(gui.mapMouseY(point.y)+gui.viewportY*32)};
            ...
            if (action.kind==TouchActionKind::BeginStroke && world().contains(point))
                gui.toolManager.handleMouseDown(gui.mapMouseX(point.x),gui.mapMouseY(point.y),gui.localTeamNo,gui.viewportX,gui.viewportY);
            else if (action.kind==TouchActionKind::Stroke && world().contains(point))
                gui.toolManager.handleMouseDrag(gui.mapMouseX(point.x),gui.mapMouseY(point.y),gui.localTeamNo,gui.viewportX,gui.viewportY);
...
void GameGUITouch::interfaceTap(ViewPoint point)
{
    ...
    if (!gui.inGameMenu && !gui.typingInputScreen && !gui.scrollableText && gui.selectionMode==GameGUI::TOOL_SELECTION && controls().contains(point)) {
        if (point.x<controls().x+controls().w/2) {
            if (preview && previewType==gui.toolManager.getBuildingName() && !globalContainer->isViewingGame()) {
    ...
    if (hudInput && layout().actions.contains(point)) {
        ...
        if (button<3) {
            ...
            panelScroll=showsBuildPalette()?0:144; clampScroll(); return;
        }
        ...
        if(showsBuildPalette()) {tapBuildPalette(point);return;}
...
void GameGUITouch::prepareDraw()
{
    ...
    if (preview) {
        const auto cursor=previewCursor();
        gui.view.mouseX=int(cursor.x); gui.view.mouseY=int(cursor.y);
        gui.mouseX=int((cursor.x-gui.camera.fractionX())*gui.camera.zoom+gui.camera.offsetX);
        gui.mouseY=int((cursor.y-gui.camera.fractionY())*gui.camera.zoom+gui.camera.offsetY);
    }
}
...
void GameGUITouch::drawPanel()
{
    ...
    auto* gfx=globalContainer->gfx;
    gfx->setClipRect();
    gfx->drawFilledRect(int(panel.x),int(panel.y),int(panel.w),int(panel.h),PhoneTheme::paper);
    if (showsBuildPalette()) {drawBuildPalette();return;}
...
void GameGUITouch::drawHUD()
{
    ...
    gfx->setClipRect();
    gfx->drawFilledRect(int(ui.status.x),int(ui.status.y),int(ui.status.w),int(ui.status.h),PhoneTheme::paper);
    // Existing unit artwork with readable free/total counts, in window-point sizing.
    {
    PhoneTheme::TextStyle countStyle(globalContainer->littleFont);
    ...
    for (int i=0;i<3;++i) {
        ...
        gfx->setUITransform(PhoneTheme::textScale*unit,x+4*unit,ui.status.y+14*unit,&clip);
    ...
    for (int i=0;i<6;++i) {
        ...
        gfx->drawFilledRect(int(x),int(ui.actions.y),int(width)-1,int(ui.actions.h),PhoneTheme::paper);
        SDL_Rect clip{int(x),int(ui.actions.y),int(width),int(ui.actions.h)};
        gfx->setUITransform(.75*unit,x+(width-24*unit)/2,ui.actions.y+2*unit,&clip);
        gfx->drawSprite(0,0,globalContainer->gamegui,icons[i]);
        gfx->setUITransform(); gfx->setClipRect();
        const char* labels[]={globalContainer->replaying ? "[Pause]" : "[Build]",globalContainer->replaying ? "[Speed]" : "[Flags]","[Info]","[Goals]","[Teams]","[Menu]"};
        drawPointLabel({x,ui.actions.y+26*unit,width,22*unit},Toolkit::getStringTable()->getString(labels[i]),.82);
    }
}
...
{
    PhoneTheme::TextStyle tutorialStyle(globalContainer->standardFont);
    ...
    gfx->drawFilledRect(int(rect.x),int(rect.y),int(rect.w),int(rect.h),PhoneTheme::paper);
    ...
    gfx->setUITransform(1.0*unit,rect.x+8*unit,rect.y+(8-tutorialScroll+first*24)*unit,&clip);
    ...
    if (gui.swallowSpaceKey) {
        gfx->drawFilledRect(int(rect.x),int(rect.y+rect.h-48*unit),int(rect.w),int(48*unit),PhoneTheme::selected);
...
{
    if (gui.selectionMode!=GameGUI::BUILDING_SELECTION || globalContainer->isViewingGame()) return nullptr;
...
void GameGUITouch::drawAllocation()
{
    ...
    for (int i=0;i<count;++i) {
        ...
        gfx->drawFilledRect(int(button.x),int(button.y),int(button.w),int(button.h),indices[i]==tab ? PhoneTheme::selected : PhoneTheme::paper);
    ...
    if (tab==1) {
        ...
        for (int i=0;i<3;++i) {
            ...
            gfx->drawFilledRect(int(button.x),int(button.y),int(button.w),int(button.h),selected ? PhoneTheme::selected : PhoneTheme::field);
    ...
    for (int side=0;side<2;++side) {
        ...
        gfx->drawFilledRect(int(button.x),int(button.y),int(button.w),int(button.h),enabled ? PhoneTheme::selected : PhoneTheme::border);
...
{
    return gui.selectionMode==GameGUI::NO_SELECTION && gui.displayMode==GameGUI::CONSTRUCTION_VIEW && !globalContainer->isViewingGame();
}
void GameGUITouch::drawBuildPalette()
{
    auto* gfx=globalContainer->gfx;const double unit=gfx->logicalUnitsPerPoint();
    const auto content=panelContent();labelClip=content;
    for(size_t i=0;i<gui.buildingsChoiceName.size();++i) {
        ViewRect row{content.x+6*unit,content.y+(i*64-panelScroll)*unit,content.w-12*unit,60*unit};
        if(row.y+row.h<=content.y || row.y>=content.y+content.h)continue;
        SDL_Rect clip{int(content.x),int(content.y),int(content.w),int(content.h)};
        gfx->setClipRect(clip.x,clip.y,clip.w,clip.h);
        gfx->drawFilledRect(int(row.x),int(row.y),int(row.w),int(row.h),gui.buildingsChoiceState[i]?PhoneTheme::field:PhoneTheme::paper);
        const auto& name=gui.buildingsChoiceName[i];
        auto* type=globalContainer->buildingsTypes.getByType(name.c_str(),0,false);
        if(type) {
            ...
            gfx->setUITransform(unit,row.x+(60-sprite->getW(frame))/2*unit,row.y+(60-sprite->getH(frame))/2*unit,&clip);
            gfx->drawSprite(0,0,sprite,frame);gfx->setUITransform();
        }
        drawPointLabel({row.x+62*unit,row.y,row.w-68*unit,row.h},Toolkit::getStringTable()->getString("["+name+"]"));
    }
    labelClip.reset();gfx->setClipRect();
    const double total=gui.buildingsChoiceName.size()*64*unit;
    if(total>content.h)gfx->drawFilledRect(int(content.x+content.w-3*unit),int(content.y+panelScroll*unit*content.h/total),int(2*unit),int(content.h*content.h/total),PhoneTheme::border);
}
void GameGUITouch::tapBuildPalette(ViewPoint point)
{
    const auto content=panelContent();if(!content.contains(point))return;
    const int row=int((point.y-content.y)/globalContainer->gfx->logicalUnitsPerPoint()+panelScroll)/64;
    if(row>=0 && row<int(gui.buildingsChoiceName.size()) && gui.buildingsChoiceState[row]) {
        gui.setSelection(GameGUI::TOOL_SELECTION,const_cast<char*>(gui.buildingsChoiceName[row].c_str()));
        panelOpen=false;
    }
}

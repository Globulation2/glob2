...
void ChooseMapScreen::updateMapInformation()
{
	...
	if (tileX > 1 || tileY > 1)
		textTemp = FormattableString("%0 x %1 (%2 x %3)").arg(mapPreview->getLastWidth() * tileX).arg(mapPreview->getLastHeight() * tileY).arg(mapPreview->getLastWidth()).arg(mapPreview->getLastHeight());
	else

...
void ChooseMapScreen::updateMapInformation()
{
	...
	if (selectedType == MAP && source.valid)
		textTemp = FormattableString("%0 %1").arg(MapTiling::colonyCount(source.header.getNumberOfTeams(), tileX, tileY)).arg(Toolkit::getStringTable()->getString("[bases]"));
	else
	...
	if (tileX > 1 || tileY > 1)
		textTemp = FormattableString("%0 x %1 (%2 x %3)").arg(w * tileX).arg(h * tileY).arg(w).arg(h);
	else
		textTemp = FormattableString("%0 x %1").arg(w).arg(h);

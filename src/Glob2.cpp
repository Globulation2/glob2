...
static int writeTiled(const std::string& mapName, int rx, int ry, int teams, int perTeam)
{
	using namespace GAGCore;
	std::unique_ptr<InputStream> stream(new BinaryInputStream(Toolkit::getFileManager()->openInputStreamBackend(mapName)));
	MapHeader source;
	if (stream->isEndOfStream() || !source.load(stream.get()))
	{
		std::cerr << "write-tiled: cannot read " << mapName << std::endl;
		return 1;
	}
	source.setFileNameOverride(mapName);
	MapHeader written = MapTiling::writeTiledMap(source, rx, ry, teams, perTeam);
	if (written.getNumberOfTeams() == 0)
	{
		std::cerr << "write-tiled: failed on " << mapName << std::endl;
		return 1;
	}
	std::cout << "write-tiled: " << written.getFileName() << " name=\"" << written.getMapName() << "\" teams=" << written.getNumberOfTeams() << std::endl;
	return 0;
}
...
int Glob2::run(int argc, char *argv[])
{
	...
	for (int ai = 1; ai + 1 < argc; ai++)
		...
		else if (strcmp(argv[ai], "-write-tiled") == 0 && ai + 5 < argc)
		{
			// -write-tiled <map file> <rx> <ry> <teams> <swarms per team>: write the
			// repeated map to the temp directory and print its path, for headless runs.
			int ret = writeTiled(argv[ai + 1], atoi(argv[ai + 2]), atoi(argv[ai + 3]), atoi(argv[ai + 4]), atoi(argv[ai + 5]));
			delete globalContainer;
			return ret;
		}

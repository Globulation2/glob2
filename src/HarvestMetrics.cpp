...
namespace
{
	...
	struct Counters
	{
		...
		Uint64 refused = 0;     // empty-tile completions refused by GLOB2_PROTO_NO_PHANTOM
...
namespace HarvestMetrics
{
	...
	void onPhantomRefused(const Unit *unit, int resource)
	{
		if (!enabled || !trackable(unit, resource))
			return;
		counters[unit->owner->teamNumber][resource].refused++;
	}
		...
		for (int t = 0; t < game.teamsCount() && t < Team::MAX_COUNT; t++)
			for (int r = 0; r < MAX_RESOURCES; r++)
			{
				...
				if (c.commits == 0 && c.harvests == 0 && c.deliveries == 0)
					continue;
				std::printf("GLOB2_HARVEST tick=%u team=%d res=%s commits=%llu harvests=%llu phantoms=%llu refused=%llu deliveries=%llu"
					...
					(unsigned long long)c.commits, (unsigned long long)c.harvests, (unsigned long long)c.phantoms, (unsigned long long)c.refused, (unsigned long long)c.deliveries,

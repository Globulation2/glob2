...
/// Maxima coordinates a shared strategic plan and specialized executors.
...
public:
  ...
  const std::vector<AITelemetry::Field> &telemetrySchema() const override
  {
	...
	/// Restore the current Maxima state after the runtime has loaded.
...
private:
	...
	///them. Pending work is serialized to retain exact order timing.

...
struct MaximaStrategy
{
	...
	struct Assault
	{
		int force_growth_ticks;
		int max_waves;
		int muster_radius;
		int muster_ready_percent;
		int muster_stall_ticks;
		int muster_max_ticks;
	} assault;
	...
	{
		int active_min_explored_percent;
		bool enabled;
	...
	struct Farming
	{
		int wheat_stock_horizon_ticks;
		int wild_wheat_yield_percent;
		bool enabled;
...
public:
	...
	/// Restore a complete configuration using the current strategy schema.

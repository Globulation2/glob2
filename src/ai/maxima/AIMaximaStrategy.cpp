...
#define BOOL_SPEC(section, member, key, unit, group, description, impact) \
	...
	const ParameterSpec parameterSpecs[] =
	{
		...
		INT_SPEC(reconnaissance, active_min_explored_percent, "recon.active_min_explored_percent", 0, 100, "percent", "recon", "Exploration coverage required before directed reconnaissance missions", StrategyImpactMedium),
		...
		INT_SPEC(farming, wheat_stock_horizon_ticks, "farming.wheat_stock_horizon_ticks", 1, 1000000, "ticks", "farming", "Horizon over which standing wheat contributes to food supply", StrategyImpactHigh),
		INT_SPEC(farming, wild_wheat_yield_percent, "farming.wild_wheat_yield_percent", 0, 100, "percent", "farming", "Share of unprotected wheat regrowth counted as supply", StrategyImpactHigh),
		...
		BOOL_SPEC(food, relocation_enabled, "food.relocation_enabled", "enabled", "food", "Rebuild an inn or swarm at a better site and retire the old one once the replacement is completed", StrategyImpactCritical),
		INT_SPEC(food, relocation_min_quality_tiles, "food.relocation_min_quality_tiles", 0, 64, "tiles", "food", "Ledger quality below which a building is never considered for relocation", StrategyImpactHigh),
		INT_SPEC(food, relocation_min_gain_tiles, "food.relocation_min_gain_tiles", 0, 64, "tiles", "food", "Realised quality improvement a relocation must offer unless it improves coverage instead", StrategyImpactHigh),
		INT_SPEC(food, relocation_min_coverage_gain_percent, "food.relocation_min_coverage_gain_percent", 0, 100, "percent", "food", "Coverage improvement a relocation must offer unless it shortens trips instead", StrategyImpactHigh),
		INT_SPEC(food, relocation_confirm_ticks, "food.relocation_confirm_ticks", 0, 1000000, "ticks", "food", "Ticks a building must stay above the quality threshold before it may be relocated", StrategyImpactHigh),
		INT_SPEC(food, relocation_cooldown_ticks, "food.relocation_cooldown_ticks", 0, 1000000, "ticks", "food", "Ticks between relocation attempts, and how long a completed replacement may wait for its deferred retirement", StrategyImpactHigh),
		INT_SPEC(food, relocation_offer_ticks, "food.relocation_offer_ticks", 0, 1000000, "ticks", "food", "Ticks a nomination is offered to the planner before it is abandoned as having no viable site", StrategyImpactHigh),
		INT_SPEC(food, relocation_payback_horizon_ticks, "food.relocation_payback_horizon_ticks", 1, 10000000, "ticks", "food", "Ticks within which a relocation's carrier savings must repay its rebuild cost", StrategyImpactCritical),
		INT_SPEC(food, relocation_cost_margin_percent, "food.relocation_cost_margin_percent", 100, 1000, "percent", "food", "Scale applied to the estimated rebuild cost, biasing toward keeping buildings", StrategyImpactHigh),
	...
	int playerNumber, ResolvedStrategy& result, std::string& error)
{
		...
		&& !gameHeader.getAIConfig(playerNumber).empty())
	{
		...
		return restoreValues(gameHeader.getAIConfig(playerNumber), result.values, error, VERSION_MINOR);
	}
    ...
    std::string& error, int versionMinor)
{
    if(versionMinor<115)
    {
        error="This Maxima save uses a retired strategy format";
        return false;
    }
    MaximaStrategy restored{};
    std::map<std::string, std::string> provenance;
    if(!applyInline("saved strategy", text, restored, provenance, error))
        return false;
    for(const auto& spec:parameterSpecs)
        if(!provenance.count(spec.key))
        {

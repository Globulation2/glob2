...
void GlobalContainer::parseArgs(int argc, char *argv[])
{
	for (int  i=1; i<argc; i++)
	{
		...
		else if(strcmp(argv[i], "--maxima-no-orders-player")==0)
		{
			int player=-1; char extra=0;
			if(i+1>=argc || sscanf(argv[i+1], "%d%c", &player, &extra)!=1 || player<0 || player>=32)
			{ fprintf(stderr,"--maxima-no-orders-player requires a player index [0,31]\n"); exit(2); }
			MaximaExperimentAudit::disableOrders(player);
			++i;
		}
		else if(strcmp(argv[i], "--maxima-audit")==0)
		{
			if(i+2>=argc) { fprintf(stderr,"--maxima-audit requires <new-jsonl-path> <identity-bundle>\n"); exit(2); }
			MaximaExperimentAudit::open(argv[i+1],argv[i+2]);
			i+=2;
		}
		else if(strcmp(argv[i], "--maxima-checkpoint-run")==0)
		{
			int additionalSteps=-1;
			if(i+2>=argc || sscanf(argv[i+2], "%d", &additionalSteps)!=1
				|| additionalSteps<=0)
			{
				fprintf(stderr, "--maxima-checkpoint-run requires <saved-game> <additional-steps>\n");
				exit(2);
			}
			runNoXGameName=argv[i+1];
			runNoXCountRuns=1;
			runNoX=true;
			runMaximaCheckpoint=true;
			automaticEndingGame=true;
			automaticEndingSteps=additionalSteps;
			automaticEndingStepsRelative=true;
			automaticGameGlobalEndConditions=true;
			disableReplayRecording=true;
			nicowarTelemetry=true;
			i+=2;
		}
		else if(strcmp(argv[i], "--maxima-checkpoint-save")==0)
		{
			int checkpointTick=-1;
			if(i+2>=argc || sscanf(argv[i+2], "%d", &checkpointTick)!=1
				|| checkpointTick<=0)
			{
				fprintf(stderr, "--maxima-checkpoint-save requires <output-game> <absolute-tick>\n");
				exit(2);
			}
			maximaCheckpointOutput=argv[i+1];
			maximaCheckpointTick=checkpointTick;
			i+=2;
		}
		else if(strcmp(argv[i], "--maxima-checkpoint-harvest")==0)
		{
			int interval=0;
			if(i+2>=argc || sscanf(argv[i+2], "%d", &interval)!=1
				|| interval<=0)
			{
				fprintf(stderr, "--maxima-checkpoint-harvest requires <directory> <interval-ticks>\n");
				exit(2);
			}
			maximaCheckpointHarvestDirectory=argv[i+1];
			maximaCheckpointHarvestInterval=interval;
			nicowarTelemetry=true;
			disableReplayRecording=true;
			i+=2;
		}
		...
		else if (strcmp(argv[i], "-test-nicowar-versions")==0)
		{
			runNicowarVersionTestGames=true;
			automaticEndingGame = true;
			automaticGameGlobalEndConditions=true;
		}
		else if (strcmp(argv[i], "-test-nicowar-versions-nox")==0)
		{
			runNicowarVersionTestGames=true;
			automaticEndingGame = true;
			runNoX=true;
		...
		else if (strcmp(argv[i], "-list-nicowar-tournament-maps")==0)
		{
			listNicowarTournamentMaps=true;
			runNoX=true;
		}
		else if (strcmp(argv[i], "-list-nicowar-scenario-maps")==0)
		{
			listNicowarScenarioMaps=true;
			runNoX=true;
		}
		else if (strcmp(argv[i], "-nicowar-tournament-reveal-map")==0)
		{
			nicowarTournamentRevealMap=true;
		}
		else if (strcmp(argv[i], "-nicowar-tournament-match")==0
			|| strcmp(argv[i], "-nicowar-tournament-match-nox")==0)
		{
			const bool runTournamentMatchWithoutGui=
				(strcmp(argv[i], "-nicowar-tournament-match-nox")==0);
			unsigned int seed=0;
			bool good=(i+4<argc);
			if(good)
			{
				nicowarTournamentMap=argv[i+1];
				good &= (sscanf(argv[i+2], "%u", &seed)==1);
				good &= (sscanf(argv[i+3], "%d", &nicowarTournamentRotation)==1);
				good &= (sscanf(argv[i+4], "%d", &automaticEndingSteps)==1);
				good &= (nicowarTournamentRotation>=0 && nicowarTournamentRotation<4);
				good &= (automaticEndingSteps>0);
			}
			if(!good)
			{
				printf("usage:\n");
				printf("-nicowar-tournament-match[-nox] <map file> <seed> <rotation 0-3> <maximum steps>\n");
				exit(1);
			}
			nicowarTournamentSeed=seed;
			runNicowarTournamentMatch=true;
			runNoX=runTournamentMatchWithoutGui;
			automaticEndingGame=true;
			automaticGameGlobalEndConditions=true;
			disableReplayRecording=runTournamentMatchWithoutGui;
			i+=4;
		}
		else if (strcmp(argv[i], "-nicowar-2v2-match-nox")==0)
		{
			unsigned int seed=0;
			bool good=(i+7<argc);
			if(good)
			{
				nicowarTournamentMap=argv[i+1];
				good &= (sscanf(argv[i+2], "%u", &seed)==1);
				good &= (sscanf(argv[i+3], "%d", &nicowar2v2AiA)==1);
				good &= (sscanf(argv[i+4], "%d", &nicowar2v2AiB)==1);
				good &= (sscanf(argv[i+5], "%d", &nicowar2v2Partition)==1);
				good &= (sscanf(argv[i+6], "%d", &nicowar2v2Swap)==1);
				good &= (sscanf(argv[i+7], "%d", &automaticEndingSteps)==1);
				good &= (nicowar2v2AiA==AI::NUMBI || nicowar2v2AiA==AI::CASTOR
					|| nicowar2v2AiA==AI::NICOWAR
					|| nicowar2v2AiA==AI::MAXIMA);
				good &= (nicowar2v2AiB==AI::NUMBI || nicowar2v2AiB==AI::CASTOR
					|| nicowar2v2AiB==AI::NICOWAR
					|| nicowar2v2AiB==AI::MAXIMA);
				good &= (nicowar2v2Partition>=0 && nicowar2v2Partition<3);
				good &= (nicowar2v2Swap==0 || nicowar2v2Swap==1);
				good &= (automaticEndingSteps>0);
			}
			if(!good)
			{
				printf("usage:\n");
				printf("-nicowar-2v2-match-nox <map> <seed> <AI A id 1,2,5,6> <AI B id 1,2,5,6> <partition 0-2> <swap 0-1> <maximum steps>\n");
				exit(1);
			}
			nicowarTournamentSeed=seed;
			runNicowar2v2TournamentMatch=true;
			runNoX=true;
			automaticEndingGame=true;
			automaticGameGlobalEndConditions=true;
			disableReplayRecording=true;
		...
		else if (strcmp(argv[i], "-nicowar-scenario-match")==0
			|| strcmp(argv[i], "-nicowar-scenario-match-nox")==0)
		{
			const bool runScenarioWithoutGui=
				(strcmp(argv[i], "-nicowar-scenario-match-nox")==0);
			unsigned int seed=0;
			bool good=(i+8<argc);
			if(good)
			{
				nicowarScenarioMap=argv[i+1];
				good &= (sscanf(argv[i+2], "%u", &seed)==1);
				good &= (sscanf(argv[i+3], "%d", &nicowarScenarioPlayers)==1);
				good &= (sscanf(argv[i+4], "%d", &nicowarScenarioCandidateAi)==1);
				good &= (sscanf(argv[i+5], "%d", &nicowarScenarioOpponentAi)==1);
				good &= (sscanf(argv[i+6], "%d", &nicowarScenarioCandidateSeat)==1);
				good &= (sscanf(argv[i+7], "%d", &nicowarScenarioPositionOffset)==1);
				good &= (sscanf(argv[i+8], "%d", &automaticEndingSteps)==1);
				good &= (nicowarScenarioPlayers>=2 && nicowarScenarioPlayers<=5);
				good &= (nicowarScenarioCandidateAi==AI::NUMBI || nicowarScenarioCandidateAi==AI::CASTOR
					|| nicowarScenarioCandidateAi==AI::NICOWAR
					|| nicowarScenarioCandidateAi==AI::MAXIMA);
				good &= (nicowarScenarioOpponentAi==AI::NUMBI || nicowarScenarioOpponentAi==AI::CASTOR
					|| nicowarScenarioOpponentAi==AI::NICOWAR
					|| nicowarScenarioOpponentAi==AI::MAXIMA);
				good &= (nicowarScenarioCandidateSeat>=0
					&& nicowarScenarioCandidateSeat<nicowarScenarioPlayers);
				good &= (nicowarScenarioPositionOffset>=0);
				good &= (automaticEndingSteps>0);
			}
			if(!good)
			{
				printf("usage:\n");
				printf("-nicowar-scenario-match[-nox] <map> <seed> <players 2-5> <candidate AI 1,2,5,7> <opponent AI 1,2,5,7> <candidate seat> <position offset> <maximum steps>\n");
				exit(1);
			}
			nicowarScenarioSeed=seed;
			runNicowarScenarioMatch=true;
			runNoX=runScenarioWithoutGui;
			// The visible alias is an interactive inspection mode, not a
			// headless fast-forward worker. Keep automatic ending only for -nox so
			// the GUI remains open long enough to inspect topology overlays.
			automaticEndingGame=runScenarioWithoutGui;
			automaticGameGlobalEndConditions=runScenarioWithoutGui;
			disableReplayRecording=runScenarioWithoutGui;
			if(!runScenarioWithoutGui)
				settings.gameSpeed=Settings::GAME_SPEED_NORMAL;
			i+=8;
		}
		else if (strcmp(argv[i], "-maxima-castor-match-nox")==0)
		{
			unsigned int seed=0;
			bool good=(i+5<argc);
			if(good)
			{
				maximaCastorMap=argv[i+1];
				good &= (sscanf(argv[i+2], "%u", &seed)==1);
				good &= (sscanf(argv[i+3], "%d", &maximaCastorTeam)==1);
				good &= (sscanf(argv[i+4], "%d", &castorTeam)==1);
				good &= (sscanf(argv[i+5], "%d", &automaticEndingSteps)==1);
				good &= (maximaCastorTeam>=0 && castorTeam>=0
					&& maximaCastorTeam!=castorTeam && automaticEndingSteps>0);
			}
			if(!good)
			{
				printf("usage:\n");
				printf("-maxima-castor-match-nox <map file> <seed> <maxima team> <castor team> <maximum steps>\n");
				exit(1);
			}
			maximaCastorSeed=seed;
			runMaximaCastorMatch=true;
			runNoX=true;
			automaticEndingGame=true;
			automaticGameGlobalEndConditions=true;
			disableReplayRecording=true;
		...
		else if (strcmp(argv[i], "-nicowar-telemetry")==0)
		{
			nicowarTelemetry=true;
		}

...
void Glob2Screen::layoutMenu(double offset)
{
    ...
    menuLayout = ResponsiveMenu::calculate({0, 0, double(getW()), double(getH())}, menuButtons.size(), minimumWidth, offset);
    ...
    menuLayout = ResponsiveMenu::calculate({0, 0, double(getW()), double(getH())}, menuButtons.size(), minimumWidth, offset, 1, minimumHeight);
    for (size_t i = 0; i < menuButtons.size(); ++i) {
...
void Glob2Screen::drawExecution()
{
    ...
    if (menuTitle.empty()) {
        ...
        const double scale = std::min((getW() - 16.0) / title->getW(), 40.0 / title->getH());
        const int width = int(title->getW() * scale), height = int(title->getH() * scale);
        gfx->drawSurface((getW() - width) / 2, (56 - height) / 2, width, height, title);
    } else {
        auto* font = Toolkit::getFont("menu");
        gfx->drawString((getW() - font->getStringWidth(menuTitle)) / 2, 16, font, menuTitle);
    }

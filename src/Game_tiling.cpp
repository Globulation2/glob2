	...
	for (int t = 0; t < mapTeams; t++)
	{
		...
		for (int i = 0; i < Building::MAX_COUNT; i++)
		{
			...
			if (!b || b->buildingState != Building::ALIVE)
				continue;
			BuildingTemplate bt = { wrapOffset(b->posX, colony.anchorX, w0), wrapOffset(b->posY, colony.anchorY, h0), b->typeNum, b->hp, b->bullets, b->maxUnitWorking, b->getMaxUnitWorkingFuture(), b->maxUnitInside, b->priority, b->unitStayRange, b->minLevelToFlag, b->receiveResourceMask, b->sendResourceMask, {}, {} };
		...
		for (int i = 0; i < Unit::MAX_COUNT; i++)
		{
			...
			if (map.getGroundUnit(u->posX, u->posY) != u->gid && map.getAirUnit(u->posX, u->posY) != u->gid)
				continue;
			UnitTemplate ut = { wrapOffset(u->posX, colony.anchorX, w0), wrapOffset(u->posY, colony.anchorY, h0), u->typeNum, u->hp, u->hungry, u->hungriness, u->experience, u->experienceLevel, u->fruitMask, u->fruitCount, u->medical, {} };

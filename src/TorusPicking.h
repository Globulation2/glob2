...
inline bool triangle(const Vertex &a, const Vertex &b, const Vertex &c, double x, double y, Hit &hit)
{
    if (a.position[3] <= 0 || b.position[3] <= 0 || c.position[3] <= 0)
        return false;
    const Vertex *vertices[] = {&a, &b, &c};
    // Shallow views produce narrow projected triangles. Keep their
    // barycentric calculation precise while sharing the float GPU vertices.
    double px[3], py[3], iw[3];
    for (int i = 0; i < 3; ++i)
    {
        iw[i] = 1.0 / vertices[i]->position[3];
    ...
    if (weights[0] < -1e-6f || weights[1] < -1e-6f || weights[2] < -1e-6f)
        return false;
    double depth = 0, reciprocalW = 0, u = 0, v = 0;
    for (int i = 0; i < 3; ++i)
    {
        double q = weights[i] * iw[i];
    ...
    if (depth < hit.depth)
        return false;
    hit = {float(u / reciprocalW), float(v / reciprocalW), float(depth)};
    return true;
}
inline bool mesh(const std::vector<Vertex> &vertices, int columns, int rows, double x, double y, Hit &hit)
{

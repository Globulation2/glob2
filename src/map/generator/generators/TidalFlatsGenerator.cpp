...
Geometry geometryFor(const GenerationRequest &r) {
  ...
  if (g.teams >= 2)
    radius = std::min(radius, (2 * g.homeRing * std::sin(pi / g.teams) - kIslandGap) / (2 * reach));
...
enum Kind { HomeIsland = 0, Neutral, Central, Sandbar, Pool, Lagoon };
struct Blob {
  Kind kind;
...
Layout design(const GenerationRequest &request, GenerationContext &context) {
  ...
  const auto fits = [&](double s0, double r0, double reach, double gap) {
    ...
    for (const Blob &b : blobs) {
      const double need = reach + b.reach + std::max(gap, b.kind == HomeIsland ? double(kIslandGap) : gap);
  ...
  scatter(Pool, int(std::lround(o.tidePools * wedgeArea / 16384.0)), 1.5, 4.0, 0.4, 4,
          "flats-pools", false);
  const RadialShape central(std::max(1.0, g.centralRadius), g.amplitude * 0.6, context, "flats-central");
...
void furnishHomes(Map &map, const Layout &L, GenerationContext &context, const TidalFlatsOptions &o) {
  ...
  for (int team = 0; team < L.g.teams; ++team) {
    ...
    const double a = std::atan2(double(t.offsetY(L.cy, L.pondY[team])), double(t.offsetX(L.cx, L.pondX[team])));
    const auto at = [&](double along, double across) {
      return std::make_pair(L.pondX[team] + int(std::lround(along * std::cos(a) - across * std::sin(a))),
    ...
    const int wheat = int(scaledCount(area * 4 / 100, o.wheat)), wood = int(scaledCount(area * 2 / 100, o.wood));
    const int total = std::min(int(farm.size()), wheat + wood);
    std::vector<int> chosen;
    for (int k = 0; k < total; ++k)
      chosen.push_back(farm[k].second);
    std::stable_sort(chosen.begin(), chosen.end(), [&](int p, int q) {
...
void stockIslands(Map &map, const Layout &L, GenerationContext &context, const TidalFlatsOptions &o) {
  ...
  for (int start = 0; start < n; ++start) {
    ...
    const auto eligible = [&](int i) { return L.islandOf[i] == -3 && clearGround(map, i % t.w, i / t.w); };
    ...
    if (const int seed = seedNear(t, mx + int(std::lround(6 * std::cos(a))), my + int(std::lround(6 * std::sin(a))), 8, eligible);
        seed >= 0) {
      const MapGeneratorPoint centre(seed % t.w, seed / t.w);
      if (L.prizeOf[start] == 0) {
        if (scaledCount(1, o.fruit) > 0)
          placeResourceClump(map, context, centre, CHERRY + int(context.bounded("flats-prizes", 3)), 2);
  ...
  if (L.g.centralRadius > 0) {
    const auto eligible = [&](int i) { return L.islandOf[i] == -2 && clearGround(map, i % t.w, i / t.w); };
    ...
    if (scaledCount(1, o.stone) > 0)
      if (const int seed = seedNear(t, L.cx + int(std::lround((L.g.centralRadius - 4) * std::cos(spin + pi / 3))),
                                    L.cy + int(std::lround((L.g.centralRadius - 4) * std::sin(spin + pi / 3))), 8, eligible);
          seed >= 0)
        placeResourceClump(map, context, {seed % t.w, seed / t.w}, STONE, 2);
  }
}
...
void seedAlgae(Map &map, GenerationContext &context, const Torus &t, int algaePercent) {
  ...
  for (int clump = 0; clump < scaledCount(int(water.size()) / 40, algaePercent); ++clump)
    placeResourceClump(map, context, water[context.bounded("flats-algae", water.size())], ALGA, 1);
}
...
bool generate(Game &game, GenerationContext &context) {
  ...
  for (int team = 0; team < teams; ++team) {
    ...
    const double a = std::atan2(double(t.offsetY(L.cy, L.homeY[team])), double(t.offsetX(L.cx, L.homeX[team])));
...
std::string validateRequest(const GenerationRequest &r) {
  ...
  if (g.homeRadius < 9)
    return "The home islands do not fit on this map; use a bigger map, fewer colonies or smaller islands.";
  return "";
}
...
std::string validateWorld(const Game &game, const GenerationContext &context) {
  ...
  for (int team = 1; team < teams; ++team) {
    ...
    if (!arrived)
      return "Colony " + std::to_string(team) + " cannot walk to colony 0 over the flats.";
  }
  if (L.g.centralRadius > 0) {
    bool arrived = false;
...
TidalFlatsOptions::TidalFlatsOptions(const GenerationRequest &r)
    : homeIslandSize(r.option("home-island-size")), extraIslands(r.option("extra-islands")),
      sandbars(r.option("sandbars")), tidePools(r.option("tide-pools")), lagoons(r.option("lagoons")),
      coastRoughness(r.option("coast-roughness")), centralIsland(r.option("central-island") != 0),

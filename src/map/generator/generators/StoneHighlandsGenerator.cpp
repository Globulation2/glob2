...
void placePonds(Layout &L, GenerationContext &context, int spacing, int pondSize)
{
	...
	for (int v = 0; v < L.valleys; ++v)
	{
		...
		const int count =
			std::max(1, std::min(1 + area / kPondArea, int(room.size()) / (2 * kMinimumPond)));
...
void furnishHome(Map &map, const Layout &L, int bootX, int bootY, int homeValley)
{
	...
	for (int side : {1, -1})
	{
		...
		const int ax = cx + int(std::lround(rx * kKitReach)),
				  ay = cy + int(std::lround(ry * kKitReach));
		...
		for (int dy = -kKitSearch; dy <= kKitSearch; ++dy)
			for (int dx = -kKitSearch; dx <= kKitSearch; ++dx)
			{
				...
				const int gapX = fx < 0 ? -fx : std::max(0, fx - 3),
						  gapY = fy < 0 ? -fy : std::max(0, fy - 3);
...
void scatterFarmland(Map &map, GenerationContext &context, const Layout &L,
					 const std::vector<int> &pondDistance,
					 const std::vector<unsigned char> &keepClear, int wheatPercent, int woodPercent)
{
	const Torus &t = L.t;
	const int n = t.w * t.h;
...
std::string validateRequest(const GenerationRequest &r)
{
	...
	if (area < std::int64_t(kAreaPerColony) * r.nbTeams)
		return "The highlands have too little room for this many colonies; use a bigger map or "
			   "fewer "
			   "colonies.";
	return "";
}

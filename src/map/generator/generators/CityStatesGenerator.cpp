...
enum HomeKind { Lakeland = 0, Riverside, Highland, Marsh, Barrens, kHomeKinds };
enum HeartKind { LakeHeart = 0, Crag, IslandHeart, Delta, ForestHeart, kHeartKinds };
...
enum Region : signed char { Sea = 0, Commons = 1, HomeLand = 2 };
...
Layout design(const GenerationRequest &request, GenerationContext &context) {
  ...
  if (depth < kFeatureDepth && (L.homeKind == Riverside || L.homeKind == Highland || L.homeKind == Barrens))
    L.homeKind = Lakeland;
  ...
  const auto outerAt = [&](double u, double d) {
    return std::min(double(g.half - 3),
                    g.outerRadius - outerBayAmp * std::max(0.0, outerBays.at(u)) *
  ...
  if (L.homeKind == Marsh)
    for (int attempt = 0; attempt < 400 && lakes.size() < 4; ++attempt) {
      const double r0 = inner + 8 + context.bounded("city-home-lakes", 1000) / 1000.0 * (depth - 16);
      const double s0 = (context.bounded("city-home-lakes", 2001) / 1000.0 - 1) * 0.6 * g.arcHalf(r0);
      RadialShape shape(2.0 + context.bounded("city-home-lakes", 1000) / 1000.0 * 2.0, 0.3, context,
                        "city-home-lakes");
  ...
  for (int attempt = 0; attempt < 400 && int(sandBlobs.size()) < homeSand; ++attempt) {
    ...
    RadialShape shape(3.0 + context.bounded("city-sand", 1000) / 1000.0 * 4.0, 0.5, context, "city-sand");
  ...
  for (int attempt = 0; attempt < 400 && int(commonsSand.size()) < wantedSand; ++attempt) {
    ...
    const double r0 = 0.45 * g.commonsRadius +
                      context.bounded("city-sand", 1000) / 1000.0 * (0.55 * g.commonsRadius - 12);
    RadialShape shape(3.0 + context.bounded("city-sand", 1000) / 1000.0 * 5.0, 0.5, context, "city-sand");
    ...
    if (r0 * std::abs(u - 0.5) * wedge < reach + g.causeway + 4 && r0 > g.commonsRadius - kLanding - reach - 4)
      continue;
    if (r0 + reach > coastAt(u, r0) - 8)
      continue;
    bool open = true;
    for (const Patch &p : commonsSand) {
      const int px = L.cx + int(std::lround(r0 * std::cos(a))), py = L.cy + int(std::lround(r0 * std::sin(a)));
  ...
  L.homeOf.assign(n, -1);
  for (auto *mask : {&L.lake, &L.river, &L.ford, &L.sand, &L.ridge, &L.strip, &L.causeway, &L.road, &L.clear})
    mask->assign(n, 0);
  ...
  for (int y = 0; y < t.h; ++y)
    for (int x = 0; x < t.w; ++x) {
      ...
      const double along = d * std::cos(theta - h.angle), across = std::abs(d * std::sin(theta - h.angle));
      if (across <= stripHalf + kLane && along >= h.coast - 3 && along <= h.coast + g.strait + 3)
        L.causeway[i] = 1;
      ...
      if (across <= stripHalf + kLane && ((along >= h.coast - kLanding && along < h.coast) ||
      ...
      if (L.region[i] == HomeLand && !L.strip[i]) {
        ...
        for (size_t seg = 0; seg + 1 < creek.size(); ++seg) {
          ...
          const double tt = std::clamp(((s - ax) * (bx - ax) + (d - ay) * (by - ay)) / std::max(1e-6, len2), 0.0, 1.0);
          const double dist = std::hypot(s - (ax + tt * (bx - ax)), d - (ay + tt * (by - ay)));
          if (dist < 1.6) {
          ...
          } else if (int(seg) == fordSegment && dist < 3.5 && std::abs(tt - 0.5) * std::sqrt(len2) < 3.5) {
            L.clear[i] = 1;
          }
        }
...
std::vector<unsigned char> seaMargin(const Map &map, const Layout &L) {
  ...
  for (int i = 0; i < n; ++i)
    sea[i] = map.getUMTerrain(i % t.w, i / t.w) == WATER && !L.lake[i] && L.region[i] != Commons;
  for (int i = 0; i < n; ++i) {
...
void furnishHomes(Map &map, const Layout &L, GenerationContext &context,
  ...
  for (int team = 0; team < L.g.teams; ++team) {
    ...
    const auto seedNear = [&](double along, double across, int within) {
      const int ax = h.lakeX + int(std::lround(along * std::cos(h.angle) - across * std::sin(h.angle)));
      const int ay = h.lakeY + int(std::lround(along * std::sin(h.angle) + across * std::cos(h.angle)));
...
void stockCommons(Map &map, const Layout &L, GenerationContext &context,
                  const CityStatesOptions &o) {
  const Torus &t = L.t;
  const int n = t.w * t.h;
  const double frontier = o.frontier / 100.0;
  const auto weight = [&](int i) {
    const double inward = std::clamp(1 - L.radius[i] / std::max(1.0, L.g.commonsRadius), 0.0, 1.0);
  ...
  if (scaledCount(1, o.fruit) > 0) {
    ...
    for (int f = 0; f < 3; ++f) {
      ...
      const int ax = L.cx + int(std::lround(rho * std::cos(a))), ay = L.cy + int(std::lround(rho * std::sin(a)));
      int seed = -1, nearest = INT_MAX;
...
void stockIslands(Map &map, GenerationContext &context, const std::vector<Island> &islands) {
  ...
  for (const Island &island : islands) {
    ...
    case 0:
      ...
      break;
    case 1:
      placeResourceClump(map, context, centre, CHERRY + int(context.bounded("city-islands", 3)), 2);
      break;
    default:
...
bool generate(Game &game, GenerationContext &context) {
  ...
  for (int team = 0; team < teams; ++team) {
    ...
    for (int i = 0; i < n; ++i)
      home[i] = L.homeOf[i] == team && !L.strip[i] && !L.clear[i] && map.isGrass(i % t.w, i / t.w);
    ...
    const double rho = std::max(h.coast + L.g.strait + 8.0,
...
std::string validateRequest(const GenerationRequest &r) {
  ...
  if (g.depth() < kMinimumDepth || g.innerArc() < g.causeway + kMinimumArc)
    return "The commons and the strait leave too little room for the home bases; use a bigger map, "
...
std::string validateWorld(const Game &game, const GenerationContext &context) {
  ...
  if (L.g.walls) {
    ...
    for (int team = 0; team < teams; ++team)
      for (int i : workers[team])
        if (landed[i] >= 0)
          return "Colony " + std::to_string(team) + "'s home can be entered from the sea.";
  }
  ...
  for (int team = 0; team < teams; ++team) {
    ...
    for (int i = 0; i < n; ++i)
      if (inside[i] >= 0 && (L.region[i] == Commons || (L.homeOf[i] >= 0 && L.homeOf[i] != team)))
        return "Colony " + std::to_string(team) + " can leave its home without its causeway, at " +
               where(i) + ".";
  ...
  if (longest - shortest > std::max(kLandingSpread, longest / 3))
    return "The colonies' walks to their landings differ by " + std::to_string(longest - shortest) +
           " steps.";
  return "";
}
} // namespace

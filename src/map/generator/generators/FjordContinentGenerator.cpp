...
FjordLayout computeLayout(Game &game, GenerationContext &context,
  ...
  return FjordLayout{W,     H,        nbTeams,       xf,     coast,
                     coreR, lakeR,    fjordInnerR,   maxStretch,
                     hasLake, lakeConnected, teamTheta};
}
...
// on the radial line.
std::vector<std::vector<MapGeneratorPoint>>
carveFjords(Game &game, GenerationContext &context, const FjordLayout &layout,
  ...
  for (int k = 0; k < layout.nbTeams; ++k) {
    ...
    double tipU = layout.fjordInnerR * cos(midTheta),
           tipV = layout.fjordInnerR * sin(midTheta);

...
double wrapped(double value, double period) { return value - period * std::floor(value / period); }
double centred(double delta, double period) { return delta - period * std::round(delta / period); }
int wrapIndex(int value, int period) { return ((value % period) + period) % period; }
...
double channelRadius(int riverWidth) { return kMinimumChannelRadius + 0.4 * (riverWidth - 1); }
...
public:
  ...
  Vec tangent(int r, int i) const {
    ...
    return normalized(Vec{centred(points[b].x - points[a].x, along), points[b].y - points[a].y});
  }
  ...
  std::vector<Vec> path(Vec from, Vec to, Vec endDirection, double meander,
    ...
    for (size_t i = 0; i < base.size(); ++i) {
      ...
      const Vec direction = normalized(base[std::min(base.size() - 1, i + 1)] - base[i ? i - 1 : 0]);
    ...
    while (true) {
      ...
      for (const Attachment &a : list) {
        ...
        for (int hop = 0; hop < 3 && added < 0; ++hop) {
          ...
          const int blocking = reaches[last.blocking].distributary ? trunk : last.blocking;
          const int next = blocking >= 0 ? nearestAttachable(blocking, last.where, sources[pick]) : -1;
          if (next < 0)
            break;
          target = blocking;
          index = next;
  ...
  for (int r : order) {
    ...
    for (const Spot &spot : spots) {
      ...
      for (const Ford &f : layout.fords)
        apart = apart && distance2(reach.points[spot.index], f.center) >= sq(f.span + span + 10);
      if (!apart)
        continue;
      chosen.push_back(spot.index);
      const Vec along = normalized(reach.points[spot.index + 2] - reach.points[spot.index - 2]);
...
Layout planLayout(int width, int height, const WatershedOptions &o, GenerationContext &context) {
  ...
  const Vec head{headU, top + std::max(0.0, apex.y - top) * between(context, kLayoutStream, 0.1, 0.3)};
  {
    Reach reach;
  ...
  for (int round = 0; round < 6; ++round) {
    ...
    for (int attempt = 0; attempt < 60 * missing && int(sources.size()) < missing; ++attempt) {
      ...
      const double hi = std::min(layout.south.at(u) - (deltaLength + southMargin), apex.y - 6);
      ...
      for (int r = trunk + 1; clear && r < int(planner.reaches.size()); ++r)
        clear = planner.distance2(s, planner.reaches[r].points.front()) >= sq(sourceSpacing);
      if (clear)
        sources.push_back(s);
    }
  ...
  for (int r = trunk + 1; r < count; ++r) {
    ...
    layout.junctions.push_back(frame.toMap(planner.reaches[reach.parent].points[reach.joinIndex]));
  }
...
template <typename Water> std::string checkChannels(const Layout &layout, Water water) {
  ...
  for (const Reach &reach : layout.reaches) {
    ...
    for (size_t i = 0; i < reach.points.size(); ++i) {
      ...
      const int x = wrapIndex(int(std::floor(p.x)), w), y = wrapIndex(int(std::floor(p.y)), h);
      if (!water(x, y))
        return "A river channel silts up at (" + std::to_string(x) + ", " + std::to_string(y) + ").";
  ...
  for (int y = 0; y < h; y += 2)
    for (int x = 0; x < w; x += 2) {
      ...
      for (size_t k = 0; open && k < layout.fords.size(); ++k) {
        const Ford &f = layout.fords[k];
        open = sq(centred(centre.x - f.center.x, w)) + sq(centred(centre.y - f.center.y, h)) >=
               sq(f.span + 12);
      }
      if (!open)
        continue;
  ...
  for (int trial = 0; trial < 32; ++trial) {
    ...
    for (size_t a = 0; a < chosen.size(); ++a)
      for (size_t b = a + 1; b < chosen.size(); ++b) {
        closest = std::min(closest, distance2(candidates[chosen[a]], candidates[chosen[b]]));
  ...
  for (int team = 0; team < teams; ++team) {
    ...
    for (int y = 0; y < h; ++y)
      for (int x = 0; x < w; ++x) {
        ...
        if (steps < 6 || steps > 22 || !map.isGrass(x, y) || !map.isResourceAllowed(x, y, CORN))
          continue;
    ...
    auto pick = [&](int nearSteps, int farSteps, bool fertile, const std::vector<Vec> &avoid,
      ...
      for (int pass = 0; pass < 3; ++pass) {
        ...
        for (const Spot &s : spots) {
          ...
          if (!apart)
            continue;
          if (!chosen || (fertile ? s.fertility > chosen->fertility : s.fertility < chosen->fertility))
            chosen = &s;
        }
        if (chosen)
          return {chosen, size};
    ...
    for (const auto &kit : {std::pair<int, int>{CORN, 2}, std::pair<int, int>{WOOD, 2},
      ...
      const auto [spot, size] = pick(stone ? 10 : 6, stone ? 14 : 10, !stone, used,
                                     stone ? 60 : 100, kit.second);
      if (!spot)
        continue;
...
void placeFarmland(Game &game, GenerationContext &context, const Tiles &t,
  ...
  for (int y = 0; y < h; ++y)
    for (int x = 0; x < w; ++x) {
      ...
      if (t.grassDepth[i] < 3 || keepClear[i] || f < floor || !map.isResourceAllowed(x, y, CORN) ||
          map.isResource(x, y))
        continue;
...
void placeStone(Game &game, GenerationContext &context, const Tiles &t,
  ...
  for (int attempt = 0; attempt < outcrops * 20 && int(placed.size()) < outcrops && !candidates.empty();
    ...
    for (const auto &p : placed)
      apart = apart && sq(centred(double(x - p.x), w)) + sq(centred(double(y - p.y), h)) >= sq(20);
    if (!apart)
      continue;
  ...
  for (size_t j = 0; j < junctions.size() && placed < groves; ++j) {
    ...
    for (int dy = -12; dy <= 12; ++dy)
      for (int dx = -12; dx <= 12; ++dx) {
        ...
        const size_t i = size_t(y) * w + x;
        if (d2 < 25 || d2 > 144 || !t.grass[i] || keepClear[i] || fertility.at(x, y) <= bestFertility ||
            !map.isResourceAllowed(x, y, CHERRY) || !reservations.free(x, y, 1))
          continue;
...
bool connectColonies(Game &game, int teams, std::string &detail) {
  ...
  for (int team = 1; team < teams; ++team) {
    ...
    if (connected)
      continue;
    std::vector<int> cost(size_t(w) * h, std::numeric_limits<int>::max()), from(size_t(w) * h, -1);
...
bool generate(Game &game, GenerationContext &context) {
  ...
  for (int team = 0; team < teams; ++team) {
    ...
    for (int dy = -6; dy <= 9; ++dy)
      for (int dx = -6; dx <= 9; ++dx) {
        const int x = wrapIndex(sites[team].x + dx, w), y = wrapIndex(sites[team].y + dy, h);
  ...
  for (const Ford &f : layout.fords) {
    ...
    for (int dy = -reach; dy <= reach; ++dy)
      for (int dx = -reach; dx <= reach; ++dx)
        if (sq(cx + dx + 0.5 - f.center.x) + sq(cy + dy + 0.5 - f.center.y) <= sq(f.span + 5))
...
std::string validate(const GenerationRequest &r) {
  if (r.nbTeams > (1 << (r.wDec + r.hDec)) / kTilesPerColony)
    return "The watershed has too little floodplain for this many colonies; use a bigger map or "
           "fewer colonies.";
  return "";
}
      ...
      for (int x = 0; x < w; ++x) {
        ...
        for (int dy = -1; dy <= 1 && !beside; ++dy)
          for (int dx = -1; dx <= 1 && !beside; ++dx)
            beside = reach[size_t(map.normalizeY(y + dy)) * w + map.normalizeX(x + dx)] >= 0;
        (type == CORN ? wheat : wood) = (type == CORN ? wheat : wood) || beside;
      }
    if (!wheat || !wood)
      return "Colony " + std::to_string(team) + " cannot walk to " + (wheat ? "wood." : "wheat.");
  }
    ...
    for (int side : {-1, 1}) {
      ...
      for (int dy = -1; dy <= 1 && !walkable; ++dy)
        for (int dx = -1; dx <= 1 && !walkable; ++dx) {
          ...
          walkable = !map.isWater(x, y) && !map.isResource(x, y) && map.getBuilding(x, y) == NOGBID;
        }
      if (!walkable)

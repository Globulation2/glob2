...
inline float GenerationNoise::randNoiseFloat()
{
	return (float)((static_cast<int>(random() & 0x7fffffffu) %
...
float GenerationNoise::Noise1d(float pos[1])
{
...
float GenerationNoise::Noise2d(float pos[2])
{
...
float GenerationNoise::Noise3d(float pos[3])
{
...
float GenerationNoise::Noise(float x, float y, float z)
{
	float p[3] = {x, y, z};
	return Noise3d(p);
}
...
void GenerationNoise::reseed(unsigned int rSeed)
{
	random.seed(rSeed);
...
void GenerationNoise::generateLookupTables()
{
	unsigned i, j, temp;

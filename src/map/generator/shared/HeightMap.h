...
/// grass later
{
	...
	unsigned oldLowerX = ~0u, oldLowerY = ~0u, oldDifferenceX = ~0u,
			 oldDifferenceY = ~0u; /// to get reproducible correlated random numbers
  public:
	enum kindOfMap
	{
		SWAMP = 0,
  ...
  private:
	...
	inline void
	lower(unsigned int coordX,
		  unsigned int coordY); /// lower lowers the region around (coordX,coordY) to min(stamp,map)
	inline void
	differenceStamp(unsigned int coordX,
					unsigned int coordY); /// multiplyStamp sets the region around (coordX,coordY)
										  /// to (1-stamp)*map if map>0 and to 1-stamp else to bias

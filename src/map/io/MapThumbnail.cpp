	...
	for (int y = 0; y < h; ++y)
		for (int x = 0; x < w; ++x)
		{
			...
			for (int sy = y0; sy < std::max(y0 + 1, (y + 1) * source.height / h); ++sy)
				...
				{
					for (int c = 0; c < 3; ++c)
						sums[c] += source.rgb[(sy * source.width + sx) * 3 + c];
					++count;
				}
			for (int c = 0; c < 3; ++c)
				out->rgb[(y * w + x) * 3 + c] = sums[c] / count;
		}
	return out;
}
} // namespace
...
void MapThumbnail::loadFromMap(const std::string &filename)
{
	*this = MapThumbnail();
	...
	{
		*this = MapThumbnail();
		std::cerr << "Map thumbnail load: " << error.what() << '\n';
	}
}
...
void MapThumbnail::encodeData(OutputStream *stream) const
{
	...
	if (image)
	{
		...
		auto small = resample(*image, std::max(1, lastW * 128 / longest),
							  std::max(1, lastH * 128 / longest));
		int ox = (128 - small->width) / 2, oy = (128 - small->height) / 2;
		for (int y = 0; y < small->height; ++y)
			for (int x = 0; x < small->width; ++x)
				for (int c = 0; c < 3; ++c)
					legacy[((x + ox) * 128 + y + oy) * 3 + c] =
						small->rgb[(y * small->width + x) * 3 + c];
	}
		...
		while (std::max(detail->width, detail->height) > 128)
		{
			...
			if (encoded.size() + 8 + member.size() <= MaxEncodedBytes)
			{
				encoded.insert(encoded.end(),
							   {'M', 'P', 'V', '2', Uint8(detail->width >> 8), Uint8(detail->width),
								Uint8(detail->height >> 8), Uint8(detail->height)});
				encoded.insert(encoded.end(), member.begin(), member.end());
				break;
			}
			detail =
				resample(*detail, std::max(1, detail->width / 2), std::max(1, detail->height / 2));
		}
	}
	...
	*this = MapThumbnail();
	try
	{
		...
		if (length == 0 || length > MaxEncodedBytes)
			throw std::runtime_error("Map preview length");
		...
		if (used < length)
		{
			...
			result->width = (p[0] << 8) | p[1];
			result->height = (p[2] << 8) | p[3];
			if (result->width < 1 || result->height < 1 || result->width > MaxResolution ||
				result->height > MaxResolution ||
				std::abs(result->width * mh - result->height * mw) > std::max(mw, mh))
				throw std::runtime_error("Map preview dimensions");
			result->rgb.resize(result->width * result->height * 3);
			if (inflateExact(encoded.data() + used + 8, length - used - 8, result->rgb.data(),
							 result->rgb.size()) != length - used - 8)
		...
		else
		{
			...
			for (int y = 0; y < result->height; ++y)
				for (int x = 0; x < result->width; ++x)
					for (int c = 0; c < 3; ++c)
						result->rgb[(y * result->width + x) * 3 + c] =
							legacy[((x + ox) * 128 + y + oy) * 3 + c];
		}
		lastW = mw;
		lastH = mh;
	...
	{
		*this = MapThumbnail();
		std::cerr << "Map thumbnail decode: " << error.what() << '\n';
	}
}

void MapThumbnail::loadIntoSurface(DrawableSurface *surface) const
{
	if (!image || !surface || surface->getW() <= 0 || surface->getH() <= 0)
		return;

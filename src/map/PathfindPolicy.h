...
// Per-team pathfinder selection. GLOB2_PATHFIND_ALT_TEAMS is a bitmask of
// team numbers that use the weighted-field pathfinder; all other teams use
// the chamfer gradient pathfinder. Unset means every team; 0 means none.
...
namespace PathfindPolicy
{
	inline std::uint32_t altTeamMask()
	{
		static const std::uint32_t mask = []() {
			...
			return env ? (std::uint32_t)std::strtoul(env, nullptr, 0) : 0xFFFFFFFFu;
		}();
		return mask;
	}

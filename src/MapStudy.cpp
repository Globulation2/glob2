...
int runMapStudy(int argc, char **argv)
{
		...
		else
		{
			...
			if (id == "dump" || id == "save" || id == "name" || id == "overlay" || id == "result")
			{
				(id == "dump"   ? dump
				 : id == "save" ? savePrefix
				 : id == "name" ? mapName
								: id == "overlay" ? overlay : resultPath) = arg.substr(eq + 1);
				continue;
			}
	...
	if (tuning)
	{
		...
		measurements << ",\"min_local_fit4\":" << minLocal << ",\"worst_wheat_distance\":" << minWheat
	...
	{
		std::ostringstream out;
		out << "{\"schema_version\":1,\"job_type\":\"generate_map\",\"status\":"
			<< Headless::quote(success ? "completed" : result.error == GenerationError::InvalidRequest ? "invalid_request" : "generation_failed")
			<< ",\"generator\":" << Headless::quote(result.generatorId)
			...
			<< ",\"workers\":" << descriptor.nbWorkers << ",\"options\":{";
		bool comma = false;
		for (const auto &option : descriptor.options)
		{
			if (comma) out << ',';
			comma = true;
			out << Headless::quote(option.first) << ':' << option.second;
		}
	...
	else if (!savePrefix.empty())
		code = saveRotatedMaps(game, result, descriptor, savePrefix,
			mapName.empty() ? "study-" + std::to_string(method) + "-" + std::to_string(seed) : mapName,
			rotations);
	if (!resultPath.empty())
	{
		resultJson.pop_back();
		resultJson += ",\"rotations\":" + std::to_string(rotations)
			+ ",\"rotations_verified\":" + ((!savePrefix.empty() && code == 0) ? "true" : "false") + "}";
		if (success && code != 0)
		{

...
int main()
{
    ...
    auto target=f.building(35,30,1);
    std::vector<Unit*> army;
    for(int i=0;i<4;++i)army.push_back(f.warrior(20+i,20,3));
    auto& ai=*f.ai; auto& context=ai.context;
    context.initialize();f.remember(target);
    ai.plan_offense(context);
    std::cout<<"WITHOUT_BARRACKS eligible="<<ai.offense_diagnostics.eligibleWarriors
      <<" slots="<<ai.offense_diagnostics.openTrainingSlots
      <<" requested="<<ai.budget.tactical_requested_force
    ...
    for(auto unit:army) {
      for(int ability=0;ability<NB_ABILITY;++ability)
        if(unit->canLearn[ability] && barracks->type->upgrade[ability])
          assert(unit->level[ability]>barracks->type->level);
    }
    ai.plan_offense(context);
    std::cout<<"WITH_UNUSABLE_BARRACKS eligible="<<ai.offense_diagnostics.eligibleWarriors
      <<" slots="<<ai.offense_diagnostics.openTrainingSlots
      <<" requested="<<ai.budget.tactical_requested_force
      <<" gate="<<ai.offense_diagnostics.gate<<"\n";

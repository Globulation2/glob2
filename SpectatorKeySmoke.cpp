...
static int run(int argc, char **argv)
{
        ...
        SDL_HideWindow(SDL_GL_GetCurrentWindow());
    {
        ...
        SDL_Keysym key{};
        key.sym = SDLK_g;
        key.scancode = SDL_SCANCODE_G;
        gui.drawAll(0);
        if (gpu) globalContainer->gfx->printScreen("spectator-flat.bmp");
        gui.handleKey(key, true, false);
        assert(gui.torusView.enabled() == gpu);
        if (gpu) {
            gui.drawAll(0);
            gui.torusView.amount = 1;
            gui.torusView.lastFrame = SDL_GetTicks();
            gui.drawAll(0);
            globalContainer->gfx->printScreen("spectator-torus.bmp");
        }
        gui.handleKey(key, true, true);
        assert(gui.torusView.enabled() == gpu);
        gui.handleKey(key, false, false);
        gui.handleKey(key, true, false);
        assert(!gui.torusView.enabled());
        gui.torusView.reset();
        globalContainer->liveSpectating = false;
        std::cout << "Spectator g: enter, ignore repeat, exit passed (gpu=" << gpu << ")\n";

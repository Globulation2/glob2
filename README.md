...
For WebAssembly, build and serve the release as described in `browser/README.md`, then run `npx playwright test tests/determinism.spec.js --project=chromium` from `browser/`. The CI comparison additionally covers Linux with GCC 11 and 13 and Windows; consult the PR checks for their status. These checks establish this scenario, not every possible simulation or generator.

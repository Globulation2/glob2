...
For WebAssembly, build and serve the release as described in `browser/README.md`, then run `npx playwright test tests/determinism.spec.js --project=chromium` from `browser/`. All four traces from [CI run 36046812951](https://github.com/Globulation2/glob2/actions/runs/36046812951) match byte for byte: Linux with GCC 11 and 13, Windows, and WebAssembly built on Linux. They also match the master macOS baseline. Compressed traces, manifests and run logs for each are included here. These checks establish this scenario, not every possible simulation or generator.

				  ...
				  GenerationContext &context)
{
	...
	for (int k = 0; k < request.nbTeams; ++k)
		for (int lake = 0; lake < o.lakes; ++lake)
		{
			...
			{
				placed = growWater(
					t, water, seed, lakeTiles, [&](int i) { return !L.buffer[i] && !water[i]; },
					[&](int i)
					{
...
{
	const Torus &t = L.t;
	// Small market-town sites on the banks: four roomy building plots, a crossroads and a
	...
	for (int k = 0; k < request.nbTeams; ++k)
	{
		...
		for (int i = 0; i < t.size(); ++i)
		{
			...
			const long long desired = t.size() / (3 * request.nbTeams);
			...
			const long long balance =
				request.nbTeams > 1 ? std::abs(homeDistance - rivalDistance) / 2 : 0;
...
bool countryRoads(Layout &L, const std::vector<int> &exits)
{
	...
	{
		std::vector<int> order = hubs;
		std::stable_sort(order.begin(), order.end(),
						 [&](int a, int b)
						 {
							 return t.dist2(exits[k] % t.w, exits[k] / t.w, a % t.w, a / t.w) <
									t.dist2(exits[k] % t.w, exits[k] / t.w, b % t.w, b / t.w);
						 });
		const int destination = order[std::min(size_t(k % 2), order.size() - 1)];
		if (!road(exits[k], destination))
		{
...
Layout design(const GenerationRequest &request, GenerationContext &context)
{
	...
	L.homes = latticeSites(t.w, t.h, request.nbTeams, context.bounded("forts-layout", t.w),
						   context.bounded("forts-layout", t.h))
				  .sites;
		...
		for (int side : {-1, 1})
			for (int dy = 4; dy <= r - 4; ++dy)
				for (int dx = 2; dx <= r - 4; ++dx)
				{
					...
					if (!rim)
						L.plot[i] = 2 * k + (side > 0);
					if (std::abs(dx - r / 2) <= 2 && std::abs(dy - r / 2) <= 2)
						L.terrain[i] = WATER;
				}
...
bool generate(Game &game, GenerationContext &context)
{
	...
	// Every market offers all three fruits on its outskirts, with room for inns inside.
	for (const auto &v : L.villages)

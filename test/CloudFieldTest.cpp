        ...
        for (int height : {2048, 4096})
            for (int time : {0, 250, 10000})
            {
                        ...
                        for (int y : {0, 192, height - 16})
                        {
                            ...
                            for (int viewportX : {0, 32, 992})
                                for (int viewportY : {0, 64, 1024})
                                {
                                    ...
                                    assert(anotherView.opacity(viewportX + localX, viewportY + localY,
                ...
                for (int draw = 0; draw < 100; ++draw)
                    assert(field.opacity(512, 640, 1.5f) == first);
            }
    std::cout
        << "World cloud coordinates, wrapped seams, magnification, and draw-independent animation passed\n";
}

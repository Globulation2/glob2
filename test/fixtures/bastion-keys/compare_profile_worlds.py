...
root=Path.cwd();base=root/'artifacts/bastion-keys';before=base/'profile-before/glob2';after=root/'build/src/glob2'
...
def run(job):
 ...
 with tempfile.TemporaryDirectory(prefix='bastion-profile-compare-') as tmp:
  for label,binary,method in [('before',before,67),('after',after,68)]:

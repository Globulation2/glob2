...
MapThumbnail decode(const std::string& bytes)
{
    ...
    MapThumbnail image; image.decodeData(&input,0); return image;
}
std::string envelope(int w,int h,const std::vector<Uint8>& bytes)
{
    auto memory=new MemoryStreamBackend;
    BinaryOutputStream output(memory);
    output.writeSint16(w,""); output.writeSint16(h,"");
    output.writeUint32(bytes.size(),""); output.write(bytes.data(),bytes.size(),"");
    return memory->takeContents();
}
std::vector<Uint8> packed(const std::vector<Uint8>& bytes)
{
    uLongf n=compressBound(bytes.size()); std::vector<Uint8> out(n);
...
{
    Map map; map.setSize(wDec,hDec,GRASS);
    ...
    MapThumbnail image; image.loadFromMap(map); return image;
}
}
struct MapPreviewHarness
{
    static void codec()
    {
        ...
        assert(restored.isLoaded() && restored.getMapWidth()==512 && restored.getMapHeight()==256);
        ...
        wire.readSint16(""); wire.readSint16("");
        ...
        for(int x=0;x<128;++x) for(int y=32;y<96;++y)
        { old[(x*128+y)*3]=x; old[(x*128+y)*3+1]=y-32; }
        auto oldImage=decode(envelope(512,256,packed(old)));
        assert(oldImage.isLoaded() && oldImage.pixels()->width==128 && oldImage.pixels()->height==64);
        ...
        auto corrupt=compressed; corrupt.back()^=0xff;
        ...
        failed.loadFromMap(""); assert(!failed.isLoaded());
        std::cout<<"PASS sharp terrain, legacy/new codecs, frame limit, malformed/short/oversized inputs and missing files\n";
    }
    static void geometry()
    {
        ...
        for (auto dimensions : {std::pair{64,512},std::pair{512,64}})
                             ...
                             G::Rect{15,20,309,38},G::Rect{15,20,89,11}})
            {
                ...
                assert(again.x==fitted.x && again.y==fitted.y &&
                       again.w==fitted.w && again.h==fitted.h);
            }
        std::cout<<"PASS rectangular placement, positive/negative multi-period drags and inverse drags\n";
    }
    static void network()
    {
        ...
        MapHeader header; header.setMapName("Preview test");
        YOGDownloadableMapInfo info(header); info.setMapID(7); info.setDimensions(512,256); info.setSize(100);
        list.receiveMessage(std::make_shared<NetDownloadableMapInfos>(std::vector<YOGDownloadableMapInfo>{info}));
        ...
        list.receiveMessage(std::make_shared<NetDownloadableMapInfos>(std::vector<YOGDownloadableMapInfo>{info}));
        assert(list.getThumbnailState("Preview test")==S::Empty);
        for(int id=0;id<50;++id) { info.setMapID(id); list.thumbnailEntry(info); }
        assert(list.thumbnailCache.size()==32);
        std::cout<<"PASS online deduplication, timeout/retry, failure, refresh cache, revision invalidation and eviction\n";
    }
    static void visuals(const std::string& output)
    {
        ...
        struct SurfaceScreen : Screen
        {
            ...
            void paint() override { gfx->drawFilledRect(0,0,gfx->getW(),gfx->getH(),Color(232,237,218)); }
        } screen(globalContainer->gfx);
        auto preview=new MapPreview(30,30,ALIGN_LEFT,ALIGN_TOP);
        preview->setDimensions(512,360); screen.addWidget(preview); screen.dispatchInit();
        auto image=terrain(9,8); preview->setMapThumbnail(image);
        auto capture=[&](const char* name)
        { screen.dispatchPaint(); globalContainer->gfx->printScreen(output+"/"+name+".bmp"); };
        ...
        auto pixel=[&](int x,int y)
        {
            ...
            Uint32 value=0;
            std::memcpy(&value,static_cast<Uint8*>(surface->pixels)+y*surface->pitch+x*surface->format->BytesPerPixel,surface->format->BytesPerPixel);
            Uint8 r,g,b; SDL_GetRGB(value,surface->format,&r,&g,&b);
        ...
        event.button.x=area.x+50; event.button.y=area.y+50; screen.dispatchEvents(&event);
        event={}; event.type=SDL_MOUSEMOTION; event.motion.state=SDL_BUTTON_LMASK;
        event.motion.x=area.x+50+area.w/2; event.motion.y=area.y+50+area.h/2; screen.dispatchEvents(&event);
        ...
        event={}; event.type=SDL_MOUSEBUTTONUP; event.button.button=SDL_BUTTON_LEFT; screen.dispatchEvents(&event);
        assert(!preview->dragging);
        auto pointUnderMouse=[&]
        {
            auto world=preview->worldArea();
            return std::array<double,2>{
                MapPreviewGeometry::wrap(double(preview->mouseX-world.x)/world.w-preview->view.offsetX),
                MapPreviewGeometry::wrap(double(preview->mouseY-world.y)/world.h-preview->view.offsetY)};
        };
        const auto anchor=pointUnderMouse();
        event={}; event.type=SDL_MOUSEWHEEL; event.wheel.y=2; screen.dispatchEvents(&event);
        auto zoomedAnchor=pointUnderMouse();
        assert(std::abs(anchor[0]-zoomedAnchor[0])<1e-12 && std::abs(anchor[1]-zoomedAnchor[1])<1e-12);
        capture("wide-zoomed");
        assert(preview->zoom>1);
        event.wheel.y=-1; screen.dispatchEvents(&event);
        ...
        event={}; event.type=SDL_MOUSEBUTTONDOWN; event.button.button=SDL_BUTTON_RIGHT;
        event.button.x=area.x+50; event.button.y=area.y+50; screen.dispatchEvents(&event);
        assert(preview->view.offsetX==0 && preview->view.offsetY==0 && preview->zoom==1);
        preview->resetView();
        preview->setMapThumbnail(terrain(8,9)); capture("tall-original");
        ...
        preview->setState(MapPreview::State::Loading); capture("online-loading");
        preview->retry=[]{};
        preview->setState(MapPreview::State::Failed); capture("online-failed");
        CustomGameScreen custom;
        custom.dispatchInit(); custom.updateLayout();
        const auto fixture=output+"/selection-fixture.map";
        std::filesystem::copy_file("maps/FourSquares1.map",fixture,std::filesystem::copy_options::overwrite_existing);
        auto start=std::chrono::steady_clock::now();
        assert(custom.loadMap(fixture));
        auto cold=std::chrono::steady_clock::now();
        for(int i=0;i<20;++i) assert(custom.loadMap(fixture));
        auto warm=std::chrono::steady_clock::now();
        std::ofstream timing(output+"/timings.txt");
        timing<<"FourSquares1.map byte-identical copy, uncached selection ms: "<<std::chrono::duration<double,std::milli>(cold-start).count()
            <<"\n20 cached selections ms: "<<std::chrono::duration<double,std::milli>(warm-cold).count()<<"\n";
        custom.activateGroup(custom.groups[0]); custom.dispatchPaint();
        globalContainer->gfx->printScreen(output+"/custom-colonies.bmp");
        MapPreview* lobby=nullptr;
        for(auto widget:custom.widgets) if(auto candidate=dynamic_cast<MapPreview*>(widget)) lobby=candidate;
        assert(lobby);
        area=lobby->mapArea();
        event={}; event.type=SDL_MOUSEBUTTONDOWN; event.button.button=SDL_BUTTON_LEFT;
        event.button.x=area.x+area.w/2; event.button.y=area.y+area.h/2; custom.dispatchEvents(&event);
        event={}; event.type=SDL_MOUSEMOTION; event.motion.state=SDL_BUTTON_LMASK;
        event.motion.x=area.x+area.w; event.motion.y=area.y+area.h; custom.dispatchEvents(&event);
        custom.dispatchPaint(); globalContainer->gfx->printScreen(output+"/custom-colonies-wrapped.bmp");
        event={}; event.type=SDL_MOUSEBUTTONUP; event.button.button=SDL_BUTTON_LEFT; custom.dispatchEvents(&event);
        custom.setup.random=true;
        custom.setup.generator.setMethodDefaults(GenerationRequest::eSWAMP);
        custom.setup.generator.wDec=9; custom.setup.generator.hDec=7;
        custom.setup.setCapacity(4);
        assert(custom.generateMap());
        MapThumbnail snapshot; snapshot.loadFromMap(custom.snapshot);
        assert(snapshot.isLoaded() && snapshot.pixels()->rgb==lobby->thumbnail.pixels()->rgb);
        std::filesystem::copy_file(custom.snapshot,output+"/generated-wide.map",std::filesystem::copy_options::overwrite_existing);
        custom.dispatchPaint(); globalContainer->gfx->printScreen(output+"/custom-wide-colonies.bmp");
        area=lobby->mapArea();
        event={}; event.type=SDL_MOUSEBUTTONDOWN; event.button.button=SDL_BUTTON_LEFT;
        event.button.x=area.x+area.w/2; event.button.y=area.y+area.h/2; custom.dispatchEvents(&event);
        event={}; event.type=SDL_MOUSEMOTION; event.motion.state=SDL_BUTTON_LMASK;
        event.motion.x=area.x+area.w; event.motion.y=area.y+area.h; custom.dispatchEvents(&event);
        custom.dispatchPaint(); globalContainer->gfx->printScreen(output+"/custom-wide-wrapped.bmp");
        event={}; event.type=SDL_WINDOWEVENT; event.window.event=SDL_WINDOWEVENT_FOCUS_LOST; custom.dispatchEvents(&event);
        assert(!lobby->dragging);
        const auto invalidated=output+"/invalidated.map";
        std::filesystem::copy_file(fixture,invalidated,std::filesystem::copy_options::overwrite_existing);
        MapThumbnail first,second; first.loadFromMap(invalidated); second.loadFromMap(invalidated);
        assert(first.isLoaded() && first.pixels()==second.pixels());
        { std::ofstream broken(invalidated); broken<<"invalid map"; }
        second.loadFromMap(invalidated); assert(!second.isLoaded());
...
int main(int argc,char** argv)
{
    ...
    GlobalContainer globals(argc>1 ? argv[1] : "glob2-map-preview-tests"); globalContainer=&globals;
    globals.runNoX=!visual; globals.settings.rememberUnit=false;
    ...
    globals.settings.screenFlags=0; globals.settings.mute=true; globals.load();
    MapPreviewHarness::geometry(); MapPreviewHarness::codec(); MapPreviewHarness::network();
    if(visual) MapPreviewHarness::visuals(argc>3 ? argv[3] : "artifacts/map-preview");

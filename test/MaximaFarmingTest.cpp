...
void MaximaFarmingTest::testSealedCoastalBarrierPorosity()
{
	using namespace AIMaxima::Farming;
	const int w=9, h=9;
	...
	CoastalBarrierPorosityResult porous=makeSealedCoastalFarmBarriersPorous(
		w,h,land,shore,interior,coastalFarm,coastalFarm,
		protectedTiles,protectedWheat);
	CPPUNIT_ASSERT_EQUAL(1, porous.sealedComponents);
	CPPUNIT_ASSERT_EQUAL(1, porous.restoredComponents);
	CPPUNIT_ASSERT(porous.openedTiles>0);
	CPPUNIT_ASSERT(std::count(protectedTiles.begin(),protectedTiles.end(),
		uint8_t(1))>0);
	...
	CoastalBarrierPorosityResult preserved=makeSealedCoastalFarmBarriersPorous(
		w,h,land,shore,interior,coastalFarm,coastalFarm,
		protectedTiles,protectedWheat);
	CPPUNIT_ASSERT_EQUAL(0, preserved.sealedComponents);
	CPPUNIT_ASSERT_EQUAL(0, preserved.restoredComponents);
	CPPUNIT_ASSERT_EQUAL(0, preserved.openedTiles);
	CPPUNIT_ASSERT(protectedTiles==before);

	// A one-tile cross-parity neck needs the deterministic residual channel.
	std::fill(land.begin(),land.end(),uint8_t(0));
	std::fill(shore.begin(),shore.end(),uint8_t(0));
	std::fill(interior.begin(),interior.end(),uint8_t(0));
	std::fill(coastalFarm.begin(),coastalFarm.end(),uint8_t(0));
	std::fill(protectedTiles.begin(),protectedTiles.end(),uint8_t(0));
	std::fill(protectedWheat.begin(),protectedWheat.end(),uint8_t(0));
	land[2*w+2]=land[2*w+3]=land[2*w+4]=1;
	shore[2*w+2]=1;
	interior[2*w+4]=1;
	coastalFarm[2*w+3]=protectedTiles[2*w+3]
		=protectedWheat[2*w+3]=1;
	CoastalBarrierPorosityResult repaired=makeSealedCoastalFarmBarriersPorous(
		w,h,land,shore,interior,coastalFarm,coastalFarm,
		protectedTiles,protectedWheat);
	CPPUNIT_ASSERT_EQUAL(1, repaired.sealedComponents);
	CPPUNIT_ASSERT_EQUAL(1, repaired.restoredComponents);
	CPPUNIT_ASSERT_EQUAL(1, repaired.openedTiles);
}

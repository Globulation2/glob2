...
void savedRelationships()
{
    ...
    // Older formats can identify issued attempts inside the live nomination,
    // but must not resurrect an earlier completed attempt for a fresh offer.
    Fixture f;
    auto& planner=f.ai->development_planner;
    planner.actionMap.at(77).replacesBuildingId=-1;
    planner.restoreRelocation(0,3000);
    assert(planner.actionMap.at(77).replacesBuildingId==0);
    planner.finishRelocation(0);
    planner.restoreRelocation(0,9000);
    assert(planner.actionMap.at(77).replacesBuildingId==-1);
}
}

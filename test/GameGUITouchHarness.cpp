...
public:
    static void run()
    {
        ...
        finger(SDL_FINGERUP,2,350,200);finger(SDL_FINGERUP,1,200,200);
        require(std::abs(gui.camera.zoom-oldZoom*1.5)<0.001,"Pinch uses the shared camera zoom");noOrder();
        flag();tap(240,240);
        const auto position=gui.camera.screenToWorld(240,240);
        require(gui.touch->preview && std::abs(gui.touch->preview->x-position.first)<1.01 && std::abs(gui.touch->preview->y-position.second)<1.01,
        ...
        for (auto [width,height] : {std::pair{320,568}, {568,320}}) {
            ...
            const auto inn=std::find(gui.buildingsChoiceName.begin(),gui.buildingsChoiceName.end(),"inn")-gui.buildingsChoiceName.begin();
            gui.touch->panelScroll=inn*64;gui.touch->clampScroll();
            const auto palette=gui.touch->panelContent();
            tap(palette.x+palette.w/2,palette.y+(inn*64-gui.touch->panelScroll+30)*unit);
            require(gui.selectionMode==GameGUI::TOOL_SELECTION && gui.toolManager.getBuildingName()=="inn",
                    "Labeled touch palette must select the same building after rotation"); noOrder();
            gui.clearSelection();
        ...
        require(globalContainer->replayReader->loadReplay("replays/touch-preview.replay"),"Replay reader loads");

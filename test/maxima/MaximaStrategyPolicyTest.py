...
class MaximaStrategyPolicyTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.maxima = (ROOT / "src/ai/maxima/AIMaxima.cpp").read_text()
    ...
    def test_strategy_consumers_do_not_read_external_configuration(self) -> None:
        consumers = "".join(
            (ROOT / name).read_text()
            for name in (
                "src/ai/maxima/AIMaxima.cpp",
                "src/ai/maxima/AIMaximaState.cpp",
                "src/ai/maxima/AIMaximaCombat.cpp",
                "src/ai/maxima/AIMaximaWorldHelpers.h",
                "src/ai/maxima/AIMaximaFarming.cpp",
                "src/ai/maxima/AIMaximaRecon.cpp",
                "src/ai/maxima/AIMaximaRuntime.cpp",
            )
        )
    ...
    def test_current_save_omits_obsolete_plans_and_phases(self) -> None:
        ...
        self.assertIn("versionMinor<115", self.state)
        save = self.state[self.state.index("void Maxima::save(") :]
    ...
    def test_maxima_keeps_id_and_owns_save_sections(self) -> None:
        ...
        self.assertIn('readEnterSection("AIMaxima")', self.state)
        self.assertIn('writeEnterSection("AIMaxima")', self.state)
        self.assertIn('readEnterSection("MaximaState")', self.state)
        self.assertIn('writeEnterSection("MaximaState")', self.state)
    ...
    def test_global_swarm_staffing_caps_are_removed(self) -> None:
        for source in (self.strategy, self.maxima,
                       (ROOT / "src/ai/maxima/AIMaximaStrategy.h").read_text(),
    ...
    def test_swarm_executor_does_not_override_controller(self) -> None:
        staffing = function(self.maxima, "void Maxima::manage_swarm")
    ...
    def test_maxima_worker_assignments_share_the_engine_limit(self) -> None:
        runtime_header = (ROOT / "src/ai/maxima/AIMaximaRuntime.h").read_text()
        runtime = (ROOT / "src/ai/maxima/AIMaximaRuntime.cpp").read_text()

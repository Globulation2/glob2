...
static void rejectedStateThrows(Game& game)
{
    Player* player=game.players[0];
    auto* storage=new MemoryStreamBackend;
    BinaryOutputStream output(storage);
    player->ai->save(&output);
    const std::string bytes(storage->getBuffer(),storage->getPosition());
    // The AI factory must reject before constructing an AI from retired settings.
    AI restored(AI::NONE,player);
    ...
    rejected=false;
    try { AI invalid(AI::MAXIMA,player); }
    catch(const std::runtime_error& error)

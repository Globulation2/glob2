...
int main()
{
	...
	// legal fallback parcel instead of treating the intent as blocked.
	{
		...
		assert(fallbackPlanner.selectAction(fallback,{small},quota,action));
		assert(action.fallbackWaterTier);
	}
...

	{
		...
		assert(colonyRestored.load(serializedInput,VERSION_MINOR));
	...
	assert(restored.load(input,VERSION_MINOR));delete input;

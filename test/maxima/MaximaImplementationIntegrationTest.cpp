...
// Current execution state round-trips; retired Maxima layouts are rejected
// before the loader consumes any bytes.
static void executionStateSaveRegression()
{
    Game game(NULL); game.map.setSize(6,6,GRASS); game.map.setGame(&game);
    game.addTeam(); game.teams[0]->race.loadDefault();
    Player player; player.setTeam(game.teams[0]);
    ...
    auto* backend=new GAGCore::MemoryStreamBackend;
    GAGCore::BinaryOutputStream output(backend);
    AIMaximaContinuation::Writer writer(&output);
    ...
    {
        GAGCore::BinaryInputStream retiredInput(
            new GAGCore::MemoryStreamBackend(bytes.data(),bytes.size()));
        retiredInput.seekFromStart(0);
        bool rejected=false;
        try { loaded.load(&retiredInput,&player,retiredVersion); }
...
static void schoolPopulationScalingRegressions()
{
    Game game(NULL);game.map.setSize(6,6,GRASS);game.map.setGame(&game);
    game.addTeam();game.teams[0]->race.loadDefault();
...
static void categoryMaintenanceCapacityRegressions()
{
    ...
    for(bool repairFirst:{false,true})
    {
        ...
        ai.budget.upgrade_level1_hospital_weight=10;
        ai.budget.upgrade_level1_racetrack_weight=10;

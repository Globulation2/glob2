...
already fills one area thins out into a newly painted one without emptying out
and refilling (`drain`), two nearby unconnected patches share as one position (`patches`), a
...
<name>`, `--ticks N` and `--warriors N` narrow a run, and `--seed N` (default 1)
sets the game's random seed, so a run repeats exactly. `--screenshots DIR` renders

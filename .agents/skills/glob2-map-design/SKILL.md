...
This skill is grounded in a code/comment review of the built-in generators. The references distinguish current mechanisms from design recommendations; they do not certify that every existing generator is fun or balanced. Paths below are relative to this skill. Follow repository `AGENTS.md`; recheck relevant code when implementing because this guide is a source map, not a frozen engine specification.
...
2. **Control the future resource footprint.** Separate guaranteed starter supplies, renewable farmland, ambient abundance, and structural deposits. Use water placement, dry zones and sand containment deliberately. Protect roads and expansion ground. For grass service areas immediately beside crops, `preventResourceGrowth` applies the existing saved `canResourcesGrow` tile flag. Keep these areas unseeded and verify final-world, actual-growth and save/load behavior; the fractal fixtures demonstrate this pattern. More wheat is not automatically better for an AI.

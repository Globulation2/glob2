...
| [Vultures](../../../../src/map/generator/generators/VulturesGenerator.cpp) | Old Growth's clearings and lakes with finite dry wheat (zero growth probability, three to five harvests a tile set after every repair) instead of forest, wood only on shores, fixed dry rations and a quarry per home, `roomyLatticeSites` for prime colony counts. | A finite-food concept must forbid renewable repairs and give fertility zero weight in the start scorer. Spend a starter budget across disconnected pockets (`growPatchesNear`) rather than failing on the first sliver. A probe that changes winners in both directions across paired seeds is not evidence for a default; a maintainer's reading of the concept (the fields are the obstacle, so fill them in) is, and the cover default went from 35% to 65% on it. |

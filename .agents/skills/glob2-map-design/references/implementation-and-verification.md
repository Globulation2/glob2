...
### Validate the route graph in more than one terrain state

For an arena that promises a circuit, global connectivity is insufficient: a chain
also connects every colony. Close all declared gates and reopen each separately,
checking that it joins its intended pair of regions (`checkGatePartition`). Include
water when the promise must survive swimming. A gate helper may virtually reopen
its supplied gate tiles, so independently verify that the actual openings are
walkable and safe from future crops.

Check both removable obstacles and future growth. Treat wheat and wood as cleared
when looking for unintended bypasses, and their possible spread as blocked when
proving permanent circulation. Eternal structural stone can stop that spread;
buildings and current fertility are not permanent barriers. For reserved building
room, remove future crop tiles from the buildable mask **before** finding complete
footprints; checking only each footprint's anchor can overcount safe space.

Test the individual promises with deliberate damage. If every court promises all
three fruits, remove only oranges and leave the other fruit intact; a total-fruit
check should not pass. Similarly, block a declared entrance while leaving its walls
...
- **A contract should survive a total reshuffle of the maps.** Write contract checks against the design's promises — the supported envelope, what the validator refuses, the guarantees every colony gets — not against fingerprints. When both constraint-solved generators had every one of their maps changed by a stream rename, all 17 contract suites still passed and only the golden rows moved, which is the split you want: golden rows say *nothing changed*, contracts say *the design still holds*.
...
- **Measure surviving features after destructive passes.** Beaches, gate clearing and decoration can erase or obstruct a feature that existed in the design mask. Count the final surviving landmark pieces, and keep ambient stone/fruit out of the entire promised junction reservation. Mutate a side tile while leaving the centre open to test a width promise; reduce wheat to a token deposit to test quantity rather than mere presence. Document when generation checks plot ownership but a standalone world validator checks only general access.
...
- **Removing a primitive.** When a rebuild orphans shared code (Bases, Compounds and Lots went with the premade bases), grep for every include and symbol, move what survives into the primitive that owns the concept (`wallStanding` into `Walls`), delete the files from `scons/sources.py`, rewrite their toolkit checks around what remains, remove definition hooks nothing uses (`startingWorkers`), their translation keys in every language, and their rows in the framework document's primitive table and in these references.
- **Profile with a fixed output corpus.** `MapGeneratorProfileFixture <profile> <seed> <rounds> [generator-id...]` drives one generator in a loop for macOS `sample` (or `perf`); read the call tree by caller, since inlined design code shows up under `generate`. Before touching anything, record a corpus of a couple of hundred requests (defaults at every shape plus random controls and each control's extremes) with `MapGeneratorStudy ... dump=... result=...`: the `STUDY` fingerprint, a hash of the tile dump and the telemetry records. Prefer deterministic terrain dumps or native fingerprints. Use saved `.map` hashes only after establishing that the chosen saving path has stable metadata across identical runs; file-format metadata can otherwise produce irrelevant differences. Check the corpus after every change, and the golden rows of every generator a shared primitive touches. Include cold single-map runs of the old and new binaries (a copy of the old `MapGeneratorStudy` built from the saved sources): the fixture's request randomization can build a design before its per-generation timer starts. Whole-process CPU time over identical fixture requests is also useful, but label that scope and distinguish it from the fixture's internal timer. Under concurrent load, report CPU time separately from wall time; sample the generator process, not a wrapping `time` process.
- **Freeze each tested build at a new path.** Use distinct executable filenames for review, games and before/after profiling, and record their source provenance. Do not overwrite an executable another test may launch. Reusing an overwritten frozen path caused launch failures during Encircled Kingdom testing on macOS; a fresh path avoided them. Keep earlier-game claims attached to their original terrain revision.
- **Optimize the extent of a query, not the growth rules.** Portage Lakes' profile found whole-map fertility fields rebuilt for local bay candidates and complete floods used for single endpoint distances. A local terrain window needs the entire query radius, the growth probe reach and the pure-tile corner halo, with correct toroidal wrapping; use a full-map fallback when the window is not cheaper. An endpoint BFS must preserve the original neighbour model, source admission and distance limit, and reset every visited scratch entry before reuse. Fifteen paired requests retained identical reported terrain/resource/space/fertility metrics; the largest seed-1 benchmark fell from 18.90 to about 4 CPU seconds. Metrics are useful evidence but do not replace fingerprints for byte-equivalence claims. Record CPU and wall time separately when other jobs share the machine, and profile again after the fix to identify the remaining cost.
- **Rollback can invalidate the loop doing the rollback.** In Portage Lakes, restoring a whole working layout after a failed optional bay freed the candidate vector being iterated. Iterate immutable candidate storage or a separate copy when the body can replace its owner. Retain the crowded seed as a repeated-generation regression; a release crash that disappears under a debugger is not evidence the bug is gone.
- **Separate opening checks from persistent invariants.** An initially unseeded inn court may legitimately grow over in a no-AI growth test; plot containment and dry structural wood must still hold. Validate opening access after all resource placement, and keep that check explicit rather than making it depend on whether a telemetry pointer was supplied. Test collection on/off for identical generation behavior.
- **The design is asked for three times.** The request check (`designFailure<design>`), `generate` and `validateWorld` each build the design from the request. Prefer the existing shared `cachedDesign` helper for a request-only design. Check that its key covers every input the layout actually uses; keep live-world validation outside the cache. A design that depends only on the request (every draw from a named stream) can be cached per thread keyed on the request, provided its telemetry is recorded into the layout and replayed into each context in order; Honeycomb isle's contract generates A, B and A again and requires the same map and records. With whole-map distance transforms replaced by local disc scans (a feature's margin over a few thousand tiles, not a transform per colony), generation became 3.5–4.4× faster with an identical 214-map corpus.
- **Reuse the helper's intermediate result before adding another cache.** Gauntlet recomputed each farm's exterior-distance flood immediately after `layFarm` had computed it. An optional `edgeDepthOut` parameter now returns that existing field. Its regression compares terrain and every returned farm field with and without the output, including empty/full regions and a field crossing the torus seam. Check existing generators' golden rows when changing a shared helper, even if its default behavior is intended to stay identical.
- **Separate output equivalence from timing evidence.** Freeze before/after binaries, alternate their order on identical requests, and retain repetitions and host-load limitations. Gauntlet's maps and complete reports matched exactly across 60 successful requests, while measured median improvements were only 2.8–5.8% under contention. A later, less-contended run was much faster; comparing it to the earlier baseline would exaggerate the optimization. Inspect the profiler stack to ensure samples cover generation rather than startup or report writing.
- **Stop a reachability proof when its goals are satisfied.** Faulted City's profile found `arrangeBuildingGrid` flooding the whole map to prove access to nearby proposed buildings. An eight-neighbour search can stop once every building has a reached cardinal face. Preserve blocked-source and torus semantics, and compare against the exhaustive predicate on fragmented masks, seam crossings, empty/duplicate sources and diagonal-only access. This optimization left paired map bytes identical and reduced measured command times by 10–17%; the [city evidence](../../../../test/fixtures/faulted-city/README.md) retains the samples and timings. Alternate old/new binaries over the same requests to reduce load-order bias. `MapGeneratorProfileFixture ... GENERATOR --largest [--telemetry]` also measures default controls at 512²/twelve teams without randomizing the request; compare collection on/off separately from JSON analysis and rendering costs.
...
- **Prove small facts with small floods, and flood from the small side.** Hidden Oasis' profile (2026-09-17) had half of generation in whole-map walking floods, most of them proving facts about one plateau: "with this pinch shut, nobody reaches the basin" flooded the whole map from the mouth once per colony, in the design and again in the validator. Flooded *from the basin outwards* it costs the basin and the sealed end of the gorge, and a leak is any reached tile off the plateau. The same turn-round served "every ledge is reached from the country" (flood from the ledge through the plateau until it leaves the rock). Bound every validator walk by the promise it checks (beyond the limit it fails either way), compute the one whole-map field the design needs once and reuse it, and do a second repair pass only where its check fails (every trail was cut twice; the second cut was needed on 2 of 2,000 maps).
- **`floodFrom` costs the map, not the flood.** It clears a whole-map step field, reserves a whole-map queue and scans a whole-map source mask on every call, and `tileMask` allocates another: a 48-step flood that reaches 4,000 tiles on a 512x512 map pays for 262,144. `reachFrom` (`Grid.h`) is the same flood, same tiles in the same order with their steps, at the cost of what it reaches (a per-thread step field put back tile by tile). Using it for the site search's yield and catchment floods and for every bounded check took a quarter off Hidden Oasis and 7% off Central Quarry with every golden row unchanged. Reach for it whenever a flood has a limit or a sealed space.
- **Measure under load with interleaved CPU time.** When something else is using the machine, wall time and the fixture's means are useless (a 285 ms mean read 445 ms ten minutes later). Keep a copy of the old binary, run old and new alternately over the same dozen requests and compare `user` seconds from `/usr/bin/time -p`. For Hidden Oasis, hashing the saved `.map` files of that request set was stable between identical runs and proved each output-preserving step byte for byte; hash the terrain dump where a generator's files are not.
- **Hoist invariants and skip provably irrelevant analysis.** Profiling Who Ate the Map? found per-tile bite rotations and detached-island service analysis even when no secondary island met the minimum grass budget. Cache sine/cosine per bite without changing projection arithmetic; skip distance fields and footprint packing only after proving all secondary capacities must remain zero. Keep returned masks correctly sized and RNG/telemetry behavior intact. Together these changes reduced paired fixture CPU time by about 12%, with identical saved maps. Separate generation-only measurements from CLI timings that include serialization and a detailed JSON report.
...
- **Do not let a failed command display yesterday's map.** Structured generation refuses an output directory that already contains a result. Check the process exit status before reading its JSON or rendering its dump; file existence alone can silently reuse old evidence. Use fresh output directories per revision, keep request/seed and binary identity with each panel, and require a complete status before launching its playtest. A gallery renderer must preserve rectangular aspect ratios and label differing tile scales; use tile dumps when native thumbnails omit the resource the design is about.
- **Use unit-cost search for unit-cost questions.** Orchard Commons' access validator spent most of its time in repeated weighted searches, although every traversable step cost one. The shared eight-neighbour BFS preserved those distances more cheaply. Match passability, source tiles, diagonal rules and unreachable values before substituting algorithms, and verify saved-map bytes as well as outcomes on a fixed corpus. Reprofile after the final terrain pass: additional noise fields and fertility work can change the cost even when the earlier hotspot stays fixed. Report telemetry on/off separately and treat wall-time comparisons under different machine loads as observations, not precise speedup claims.
- **Freeze the executable for long studies.** Copy the completed binary before launching
  games or sweeps; a rebuild can briefly remove the live executable between games in a
  shell loop. Record the frozen revision and reuse the same requests after a fix.
  Flush each result row, distinguish infrastructure failures (including disk exhaustion)
  from generator refusals, and resume missing requests without replacing failed seeds.
  Compress completed logs and saves; do not delete another task's artifacts to make room.
- **Bound expensive geometry without changing it.** Last Treeline evaluated shoreline
  trigonometry across the entire map. The sum of its radial terms gave a strict outer
  bound, so distant tiles could skip the angle calculation. Preserve comparisons and
  RNG calls, then compare actual saved-map fingerprints across seeds and dimensions.
  Pair before/after measurements on the same machine load and report CPU as well as wall
  time; an earlier busy-machine wall time is not a defensible speedup baseline.
...
- **Linux golden rows for a new generator.** Current `--require-rows` fails when a playable generator has no rows for the platform. CI's failure output prints the observed rows; retrieve the completed job's logs (the per-job logs API also works while other jobs are running), verify the generator ID and request columns, and add only the new generator's rows. Compare observations across supported Linux compilers before treating them as one platform baseline. Alternatively run `MapGeneratorGoldenTest <profile-dir> --update` on a Linux host and copy only the intended rows. Gauntlet's first integration run caught this missing-baseline case while every supported sweep cell passed; rerun CI after adding the observed rows.

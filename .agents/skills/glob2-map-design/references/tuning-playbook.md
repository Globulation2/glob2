...
| Identical modules everywhere (an archipelago whose islands are all one size and one rounded square) | Repetition reads as manufacture even when every module is right | Deal each module a shape class from a named stream where there is room to spare: keep a third at the nominal shape so the design's guarantees (a full island for every home) still hold, and let the rest vary in size, stretch, squareness and wobble; keep every module nominal on a crowded map |

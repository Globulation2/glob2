...
- **Dead ranges.** Wheat amount above 100% changed nothing because the fields were already full; wood saturated at 200%. Cap a percentage at the value where it stops mattering (`GeneratorControl::percentage(id, label, maximum)`), or remap the planting curve when the intended range should progressively fill spare capacity. Measure every adjacent step after changing the curve; Faulted City first saturated between 275% and 300%, even though its endpoints differed.

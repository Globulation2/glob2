...
Measure it the only way it shows: count wood tiles on a final save against the same count at tick zero, or look at the game. Then choose one of the three containments the engine actually offers, and know which one you are using:

- **Ground where the probe fails.** A deposit whose growth chance is zero — dry land, or sand behind it — never spreads, however fertile its neighbours. Finite stock by geography.
- **Sand around the plot.** A deposit cannot occupy sand at all, so a sand-capped plot contains its own crops permanently. This is terrain, not a flag.
- **The saved no-growth flag, over the patch *and its whole ring*.** `canResourcesGrow` blocks growth **on the flagged tile only**. Flagging a copse and nothing else still lets it extend into the unflagged ground beside it: the flag has to cover the patch plus the tiles it could reach (`preventResourceGrowth` over a dilated mask). And the ring has to be complete. Some ground cannot carry the flag — a fruit court's own tiles, whose fruit must grow back — and carving those out of the ring leaves exactly that side open. Keep the patch away from such ground instead, so the ring never needs an exception. The fractal maps learned both halves on 2026-09-16: flag on the copses alone, and the maps grew over; ring with the courts and module margins carved out, and one game still filled the orchard island with forest and bloomed off three module corners. With a complete ring and copses kept clear of courts and crops, the same seeds went from 27–32% of the map in wood after 50,000 ticks to about 4%.
- **The flag over the ring alone, for renewable food that must not spread.** The engine checks the flag on the tile that would change: the patch's own tile when it thickens, the neighbour when it extends. Flag only the ring and a patch regrows in place after every harvest yet never extends — the containment for a spot of wheat on a bank where a sand-capped plot would look wrong. The fractal maps' shoreline wheat uses it; in a 50,000-tick game its largest wheat patch stayed at plot size.

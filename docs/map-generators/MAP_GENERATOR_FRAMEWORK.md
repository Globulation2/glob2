...
| `Topology` | `connectedRegions` component labelling with explicit wrap and neighbour policy; `regionAdjacency` and `graphDistances` over sparse labels; `DisjointSets`, union-find for a spanning tree of crossings grown by Kruskal's rule |

...
![Wheat: classic and recovered original](images/ressource13.png)
...
4. **Keep successful references fixed.** `pool0b0` keeps its approved baseline bytes; `school1b0` now uses the recovered original instead of its experimental baseline. Walls retain the conservative treatment to preserve connected borders and translucent markings. `finishing.json` records the per-image selection, reference locks and review decisions.

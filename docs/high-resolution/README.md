...
The new masks produce smoother shoreline shapes. Review the connected patch and in-game shoreline screenshot alongside the original enlarged artwork. Run `connected_terrain.py` before exporting the pack; it uses the retained grass/sand material sources and the engine lookup table.

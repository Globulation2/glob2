...
6. **Treat terrain conservatively.** All 272 terrain variants are repeated 5×5 before inference, then center-cropped. Model detail is capped at ±6 channel values and mixed at 20%; it fades to zero at borders, retaining at least four output pixels of the bicubic source edge exactly. This preserves existing joins. Terrain improvement is deliberately modest. Shoreline transitions use the same edge constraints. Water receives restrained interior detail with its boundary retained. Each runtime atlas mip level is built separately per tile, then border-extruded and packed to avoid neighboring-frame sampling.

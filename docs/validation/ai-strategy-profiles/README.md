...
The strengths sections describe strategic capabilities rather than naming favorable maps. Castor's food-capacity birth controls are in `castor/Control.cpp::controlSwarms`; Numbi's food/healing/training progression is in `AINumbi.cpp::getOrder`. The other strengths refer to the previously cross-checked production, conversion, wave-support, recruitment and scouting mechanisms. Map associations do not establish that these mechanisms caused the outcomes; no such causal claim is made. The revised wording is descriptive and does not claim superior efficiency or guaranteed resilience. Named maps are confined to this evidence document; the game describes strategic dependencies, broad situations and terrain categories.

### Situation-based weaknesses

Named maps have been removed from every in-game summary, short description and profile. Castor's food-lock branch can stop births; Nicowar's war phase requires trained warriors; Cabino's attack module requires barracks and sufficient trained units; Cortex's supported waves rely on forward provisioning. Numbi's staged buildup offers a plausible early-pressure opportunity, not a demonstrated human counter-strategy. Warrush retains only the broad river-heavy-terrain association. These descriptions do not infer a universal terrain weakness from individual generator results.
